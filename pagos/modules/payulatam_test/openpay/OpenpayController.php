...
{ 
	...
	$customerData = array(
     ...
     'phone_number' => $address->phone_mobile.' '.$address->phone,
     'address' => array(
...
public function add_charge($post,$contador){
	...
	$card = array(
    				'card_number' => $post['numerot'],
    				'holder_name' => $post['nombre'],
    				'expiration_year' => substr($post['date'], 2,2),
    				'expiration_month' => substr($post['date'], 5,2),
    				'cvv2' => $post['codigot'] );

$chargeRequest = array(
    					'method' => 'card',
    					'source_id' => $post["token_id"],
    					'amount' => $this->context->cart->getOrderTotals()['total_orden'],
    					'currency' => 'MXN',
    					...
    					'metadata' => array(
      										...
      										'address3' 		=> $address->other,
      										'phone'			=> $address->phone_mobile .' '.$address->phone,
      										'fecha_compra' 	=> date("Y-m-d H:i:s"), 
      										'total' 		=> $this->context->cart->getOrderTotals()['total_orden'],
      										'descuento' 	=> $this->context->cart->getOrderTotals()['total_descuento'],
...
protected function add_transaction($trans){
	...
	echo "<br> tx: ". $sql="INSERT INTO `"._DB_PREFIX_."openpay_transaction` (`id`,
	 `authorization`,
	 `transaction_type`,
	 `operation_type`,
	 `method`,
...
3004 => 'La tarjeta ha sido identificada como una tarjeta robada, Se ha dato aviso a la autoridad competente.',
3005 => 'La tarjeta ha sido identificada como una tarjeta fraudulenta, La operación no esta permitida para este cliente o esta transacción, Se ha dato aviso a la autoridad competente.',
3008 => 'La tarjeta no es soportada en transacciones en linea.',
3009 => 'La tarjeta fue reportada como perdida, Se ha dato aviso a la autoridad competente.',
3010 => 'El banco ha restringido la tarjeta.',
3011 => 'El banco ha solicitado que la tarjeta sea retenida. Contacte al banco, Se ha dato aviso a la autoridad competente.',
