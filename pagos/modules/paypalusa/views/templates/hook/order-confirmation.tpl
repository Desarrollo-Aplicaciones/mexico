...
<div class="conf confirmation">
	{literal}¡Felicitaciones!, tu pago esta en estado de verificación, to orden se a generado con la refencia: {/literal} {if isset($paypal_usa_order.reference)} <b>{$paypal_usa_order.reference|escape:html:'UTF-8'}</b>{else} {l s='the ID' mod='paypalusa'} <b>{$paypal_usa_order.id|escape:html:'UTF-8'}</b>{/if}.
</div>
{else}
<div class="error">
