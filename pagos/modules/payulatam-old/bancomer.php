...
class Bancomer extends PayUControllerWS{    
...
if (isset($_POST['pagar']))
  {
    ...
    $fechaBANCOMER=date ( 'Y-m-d' , $nuevafecha ).'T'.date ( 'h:i:s' , $nuevafecha );          
    $keysPayu= $conf->keys();

    // Script Json payuLatam (BANCOMER)   
...
if($response['code'] === 'ERROR')
{
 ...
 $conf->failed_transaction(0.0, $customer->id, $data, $response, 'BANCOMER', $response['code'], $this->context->cart->id,$id_address);
 $messsage='<h1>¡Error!, Transaccion Rechazada</h1><br>'.$response['error'];    
        ...
        elseif ($response['code'] === 'SUCCESS' && $response['transactionResponse']['state'] === 'PENDING' && $response['transactionResponse']['responseMessage'] != 'ERROR_CONVERTING_TRANSACTION_AMOUNTS') {
            ...
            $extra_vars =  array('method'=>'BANCOMER',
                            ...
                            $this->createPendingOrder($extra_vars, 'BANCOMER', 'El sistema esta en espera de la confirmación de la pasarela de pago.');
                            ...
                            $conf->pago_payu($order['id_order'], $customer->id, $data, $response, 'BANCOMER', $extras, $id_cart,$id_address);
      ...
      $conf->failed_transaction(0.0, $customer->id, $data, $response, 'BANCOMER', $response['transactionResponse']['state'], $id_cart,$id_address);
