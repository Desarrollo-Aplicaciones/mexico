...
$conf->error_payu($id_order, $customer->id, $data, $response, 'IXE', $response['transactionResponse']['state'], $this->context->cart->id, $id_address); 
 $messsage='<h1>¡Error!, Transaccion Rechazada</h1><br>'.$response['error'];    
            ...
            elseif ($response['code'] === 'SUCCESS' && $response['transactionResponse']['state'] === 'PENDING' && $response['transactionResponse']['responseMessage'] != 'ERROR_CONVERTING_TRANSACTION_AMOUNTS') {
                    ...
                    $this->createPendingOrder($extra_vars, 'IXE', 'El sistema esta en espera de la confirmación de la pasarela de pago.', 'PAYU_WAITING_PAYMENT');
