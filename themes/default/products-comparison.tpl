			...
			<td width="{$width}%" class="ajax_block_product comparison_infos">
				...
				{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
					{if ($product->quantity > 0 OR $product->allow_oosp)}
						<a class="exclusive ajax_add_to_cart_button" rel="ajax_id_product_{$product->id}" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")|escape:'html'}" title="{l s='Agregar al carrito'}"><span></span>{l s='Agregar al carrito'}</a>
					{else}
						<span class="exclusive">{l s='Agregar al carrito'}</span>
					{/if}
				{else}
