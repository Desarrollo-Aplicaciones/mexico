...
<div id="product_{$product.id_product}_{$product.id_product_attribute}_0_{$product.id_address_delivery|intval}{if !empty($product.gift)}_gift{/if}" class="listado_carrito {if isset($productLast) && $productLast && (!isset($ignoreProductLast) || !$ignoreProductLast)}last_item{elseif isset($productFirst) && $productFirst}first_item{/if} {if isset($customizedDatas.$productId.$productAttributeId) AND $quantityDisplayed == 0}alternate_item{/if} cart_item address_{$product.id_address_delivery|intval} {if $odd}odd{else}even{/if}" style="margin-left: 0px;margin-right: 0px;position: relative;">
	...
	<div class="cart_product">
		<a id="imagenProductoCompra" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'htmlall':'UTF-8'}" title="{$product.name|escape:'htmlall':'UTF-8'}"><img id="imagproducto" src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'medium_default')}" alt="{$product.name|escape:'htmlall':'UTF-8'}"/></a>
	</div>
	<div class="cart_description" id="ProductoDescription">
		<p class="s_title_block producto"><a id="descripcioNombre" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'htmlall':'UTF-8'}">{$product.name|truncate:50:'...'|lower|capitalize|escape:'htmlall':'UTF-8'}</a></p>
	...
	<div class="cart_unit" id="valorUnidad">
		<span class="labels" id="labels">Precio: </span>
		...
		{else}
			...
			{if !isset($customizedDatas.$productId.$productAttributeId) OR $quantityDisplayed > 0}
					<div class="cantinterna">
						...
						<a rel="nofollow" class="cart_quantity_up" id="cart_quantity_up_{$product.id_product}_{$product.id_product_attribute}_0_{$product.id_address_delivery|intval}" href="{$link->getPageLink('cart', true, NULL, "add&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_address_delivery={$product.id_address_delivery|intval}&amp;token={$token_cart}")}" title="{l s='Add'}"><img src="{$img_dir}icon/quantity_up_3.jpg" alt="{l s='Add'}" class="boton"/></a>	
	...
	<div class="cart_total" id="totalCart">
		<span class="labels" id="labels">Sub Total: </span>
	...
	{if !isset($noDeleteButton) || !$noDeleteButton}
		<div class="cart_delete">
			<span class="labels" id="separador"><img src="{$img_dir}vert_sep.jpg"/></span>
		{if (!isset($customizedDatas.$productId.$productAttributeId) OR $quantityDisplayed) > 0 && empty($product.gift)}
			...
			<a rel="nofollow" id="{$product.id_product}_{$product.id_product_attribute}_0_{$product.id_address_delivery|intval}" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_address_delivery={$product.id_address_delivery|intval}&amp;token={$token_cart}")}" style="transition:none;">
				<span class="labels" id="labels">
					{l s='Delete'}
				</span>
				<span class="labels" id="labels" style="display:initial;">&nbsp;x&nbsp;</span>
...
<div><img src="{$img_dir}hor_sep.jpg" width="100%" height="1px"/></div>
</div>
