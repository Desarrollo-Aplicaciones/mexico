...
<div class="block-center" id="block-history">
	...
	<table id="order-list" class="std">
		<thead>
			<tr>
				<th class="first_item"><span>{l s='Order reference'}</span></th>
		...
		{foreach from=$orders item=order name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				<td>
					<a class="color-myaccount" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true)|escape:'html'}');">
						<img src="{$img_dir}my-account/details.jpg" alt="{l s='details'}" title="{l s='details'}" class="icon" />
					</a>
					<span>{if isset($order.invoice) && $order.invoice && isset($order.virtual) && $order.virtual}<img src="{$img_dir}icon/download_product.gif" class="icon" alt="{l s='Products to download'}" title="{l s='Products to download'}" />{/if}
				...
				<td class="history_price"><div class="price">{displayPrice price=$order.total_paid currency=$order.id_currency no_utf8=false convert=false}</div></td>
				...
				<td class="history_detail">
					...
					{/if}
						...
						<img src="{$img_dir}my-account/rebuy.png" alt="{l s='Reorder'}" title="{l s='Reorder'}" class="icon" />
					</a>
				</td>
			</tr>
		{/foreach}
