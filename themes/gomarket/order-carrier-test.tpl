...
{else}
	<h3 class="carrier_title">{l s='Choose your delivery method'} - paso 1</h3>
	...
	<div class="delivery_options_address" >
            {if isset($delivery_option_list)} <h2>- paso 2</h2>
		{foreach $delivery_option_list as $id_address => $option_list}
			...
			{foreach $option_list as $key => $option}
                            <div class="delivery_option {if ($option@index % 2)}alternate_{/if}item" >
                                  ...
                                  <!-- no --> <input class="delivery_option_radio" type="radio" name="delivery_option[{$id_address}]" onchange="{if $opc}updateCarrierSelectionAndGift();{else}updateExtraCarrier('{$key}', {$id_address});{/if}" id="delivery_option_{$id_address}_{$option@index}" value="{$key}" {if isset($delivery_option[$id_address]) && $delivery_option[$id_address] == $key}checked="checked"{/if} />
						...
						<table class="resume">
							<tr>
								...
								{if $option.unique_carrier}
									...
									<!-- no -->	<div class="delivery_option_delay">{$carrier.instance->delay[$cookie->id_lang]}</div>
									{/if}
								{/if}
