...
<div id="cart_block" class="block exclusive">
	<h4 class="title_block">
		<a href="{$link->getPageLink("$order_process", true)|escape:'html'}&paso=inicial" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">{l s='Cart' mod='blockcart'}</a>
		{if $ajax_allowed}
	...
	{if $discounts|@count > 0}
						...
						<tr class="bloc_cart_voucher" id="bloc_cart_voucher_{$discount.id_discount}">
							...
							<td class="delete">
								{if strlen($discount.code)}
									<a class="delete_voucher" href="{$link->getPageLink('$order_process', true)}?deleteDiscount={$discount.id_discount}&paso=inicial" title="{l s='Delete' mod='blockcart'}" rel="nofollow"><img src="{$img_dir}icon/delete.gif" alt="{l s='Delete' mod='blockcart'}" class="icon" /></a>
								{/if}
							</td>
						</tr>
						{/if}
		...
		<p id="cart-buttons">		
			...
			{if $order_process == 'order'}<a href="{$link->getPageLink("$order_process", true)}&paso=inicial" class="button_small" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">{l s='Cart' mod='blockcart'}</a>{/if}
			<a href="{$link->getPageLink("$order_process", true)}&paso=inicial" id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">{l s='Check out' mod='blockcart'}</a>
		</p>
	</div>
	</div>
</div>
