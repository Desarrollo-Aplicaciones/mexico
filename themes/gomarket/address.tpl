...
<form action="{$link->getPageLink('address', true)|escape:'html'}" method="post" class="std" id="add_address">
	<fieldset>
		<h3>{if isset($id_address)}{l s='Your address'}{else}{l s='New address'}{/if}</h3>
			...
			<input type="hidden" class="text" name="dni" id="dni" value="{if isset($smarty.post.dni)}{$smarty.post.dni}{else}{if isset($address->dni)}{$address->dni|escape:'html'}{/if}{/if}" />
	...
	{* $field_name} : {$address->{$field_name} *}
		...
		<p class="required text">
			...
			<input type="text" name="firstname" id="firstname" value="{if isset($smarty.post.firstname)}{$smarty.post.firstname}{else}{if isset($address->
			firstname)}{$address->firstname|escape:'html'}{/if}{/if}" />
		...
		<p class="required postcode text">
			...
			<input type="text" id="postcode" maxlength="5" name="postcode" value="{if isset($smarty.post.postcode)}{$smarty.post.postcode}{else}{if isset($address->postcode)}{$address->postcode|escape:'html'}{/if}{/if}" onkeyup="$('#postcode').val($('#postcode').val().toUpperCase());" />
		</p>

		<p class="required postcode text">
			<label for="id_colonia">{l s='Colonia'} <sup>*</sup></label>
			<select name="id_colonia" id="id_colonia">
				<option value="">-</option>
				{$address.colonia_list}
			</select>
		</p>
		...
		<p class="required id_state select">
			...
			<select name="id_state" id="id_state" OnChange="ciudad_s();">
				<option value="">-</option>
			</select>
		</p>
		{/if}
		...
		<p class="required city select">
			...
			<select name="city" id="city" >			
				<option value="">-</option>
				{if isset($address->address_city_list) && $address->address_city_list != ''}
					{$address->address_city_list}
				{else}
					<option value="{if isset($smarty.post.city)}{$smarty.post.city}{else}{if isset($address->city)}{$address->city|escape:'html'}{else}{/if}{/if}">{if isset($smarty.post.city)}{$smarty.post.city_id}{else}{if isset($address->city)}{$address->city|escape:'html'}{else}-{/if}{/if}</option>
				{/if}
			</select>
		</p>
		{*
		...
		*}
		...
		{/if}
		<input type="hidden" name="city_id" id="city_id" value="{if isset($smarty.post.city)}{$smarty.post.city_id}{else}{if isset($address->city_id)}{$address->city_id|escape:'html'}{/if}{/if}"/>
		<p class="textarea">
	...
	<p class="submit2">
		...
		<input type="submit" name="submitAddress" id="submitAddress" value="{l s='Save'}" class="button" OnClick="ciudad_s();"/>
	</p>
</form>
