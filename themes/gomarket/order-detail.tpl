...
{if $carrier->id}<p><strong>Transporte:</strong> {if $carrier->name == "0"}{$shop_name|escape:'htmlall':'UTF-8'}{else}{$carrier->name|escape:'htmlall':'UTF-8'}{/if}</p>{/if}
<p><strong>Método de pago:</strong> <span class="color-myaccount">{$order->payment|escape:'htmlall':'UTF-8'}</span></p>
{if $invoice AND $invoiceAllowed}
<p>
	<img src="{$img_dir}icon/pdf.gif" alt="" class="icon" />
	<a target="_blank" href="{$link->getPageLink('pdf-invoice', true)}?id_order={$order->id|intval}{if $is_guest}&secure_key={$order->secure_key}{/if}">Descargar tu factura en formato PDF</a>
</p>
{/if}
{if $order->recyclable}
...
<h3>Sigue tu orden paso a paso</h3>
...
{if !$is_guest}
	...
	<form action="{$link->getPageLink('order-detail', true)}" method="post" class="std" id="sendOrderMessage">
		<h3>Agrega un mensaje</h3>
		<p>Si quieres estás interesado puedes agregar un comentario sobre tu orden, por favor escríbela abajo.</p>
		<p>
