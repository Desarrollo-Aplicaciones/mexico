...
<div id="encabezados" >
	<img src="{$img_dir}authentication/encabezados.png" style="width:100%;display:block; margin: auto;padding: 10px 0px;"/>
</div>
...
<input type="text" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email|stripslashes}{/if}" /><br>
...
<div style="display:table-cell;float:right;margin-top: 10px;"><a class="olvida" style="font-size: 9pt;" href="{$link->getPageLink('password')}">{l s='Forgot your password?'}</a></div>
</div>
</fieldset>
</form>
</div>
...
<ul>
<li style="height: 54px;"><div><img src="{$img_dir}authentication/g644.png" id="seguridadImagen"/></div></li>
</ul>
...
<p>
<div style="display:inline-block;width: 20%;margin-left: 6px;"><img src="{$img_dir}authentication/amex.png" style="display:block; margin: auto;"/></div>
<div style="display:inline-block;width: 26%;"><img src="{$img_dir}authentication/visa.png" style="display:block; margin: auto;"/></div>
<div style="display:inline-block;width: 22%;"><img src="{$img_dir}authentication/master.png" style="display:block; margin: auto;"/></div>
<div style="display:inline-block;width: 22%;"><img src="{$img_dir}authentication/diners.png" style="display:block; margin: auto;"/></div>
</p>
<p>
<div style="display:inline-block; width: 33%;"><img src="{$img_dir}authentication/pse.png" style="display:block; margin-bottom: 7px; margin-left: auto; margin-right: auto;"/></div>
<div style="display:inline-block; width: 33%;"><img src="{$img_dir}authentication/baloto.png" style="display:block; margin: auto; "/></div>
<div style="display:inline-block; width: 33%;"><img src="{$img_dir}authentication/cod.png" style="display:block; margin: auto;"/></div>
</p>
</div>
</div>
</div>
...
<div style="display: block; width:100%;">
                    {if  isset($GUEST_FORM_ENABLED) &&$GUEST_FORM_ENABLED }  
                         ...
                         <samp class="titulo"><input type="radio" id="opinvitado" name="opcompra"   {if isset($comprarapida)}  checked="checked"  {/if}>Compra rapida</samp>
                      {else}
...
<form action="{$link->getPageLink('authentication', true)}" method="post" id="account-creation_form">
{$HOOK_CREATE_ACCOUNT_TOP}
<fieldset>
<div>
<div style="diplay:table-cell; float:left; width: 25%;">
<p style="font-size:8pt;" id="label-gender">{l s='Sexo*:'}</p>
{foreach from=$genders key=k item=gender}
<input type="radio" style="font-size:8pt;" name="id_gender" id="id_gender{$gender->id}" value="{$gender->id}" {if isset($smarty.post.id_gender) && $smarty.post.id_gender == $gender->id}checked="checked"{/if} />
<label style="font-size:8pt;" for="id_gender{$gender->id}">{$gender->name}</label>
{/foreach}
</div>
<div style="display:table-cell; float:left; width: 74%;">
<p style="font-size:8pt;" id="label-email">{l s='E-mail'}*:</p>
<input type="text" style="width: 92%;" id="reg-email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email}{/if}" />
</div>
</div>
<div>
<div style="diplay:table-cell; float:left; width: 48%;">
<p style="font-size:8pt;" id="label-passwd">{l s='Password'}*:</p>
<input type="password" style="width: 90%;" name="passwd" id="passwd" selector="clave"/>
</div>
<div style="display:table-cell; float:left; width: 48%; ">
<p style="font-size:8pt;margin-left: 10px;" id="label-conf">{l s='Confirma Contraseña*:'}</p>
<input type="password" style="width: 90%; float:right;" name="conf-passwd" id="conf-passwd" selector="confirma"/>
</div>
</div>
<div>
<div style="diplay:table-cell; float:left; width: 48%; ">
<p style="font-size:8pt;" id="label-first">{l s='First name'}*:</p>
<input onkeyup="$('#firstname').val(this.value);" style="width: 90%;" type="text" id="customer_firstname" name="customer_firstname" value="{if isset($smarty.post.customer_firstname)}{$smarty.post.customer_firstname}{/if}" />
</div>
<div style="display:table-cell; float:left; width: 48%; ">
<p style="font-size:8pt;margin-left: 10px;" id="label-last">{l s='Last name'}*:</p>
<input onkeyup="$('#lastname').val(this.value);" style="width: 90%; float:right;" type="text" id="customer_lastname" name="customer_lastname" value="{if isset($smarty.post.customer_lastname)}{$smarty.post.customer_lastname}{/if}" />
</div>
</div>
<div>
<div style="diplay:table-cell; float:left; width: 48%; ">
<p style="font-size:8pt;" id="label-id">{l s='Tipo documento*:'}</p>
...
<div style="display:table-cell; float:left; width: 48%; ">
<p style="font-size:8pt; margin-left: 10px;" id="label-dni">{l s='Identificacion*:'}</p>
<input type="text" style="width: 90%; float:right;" id="dni" name="dni" />
</div>
</div>
<div>
<div style="diplay:table-cell; float:left; width: 100%; ">
<p style="font-size:8pt;" id="label-birth">{l s='Date of Birth'}*:</p>
<div>
...
<label for="newsletter" style="font-size:8pt;">{l s='Inscribirse al Boletín'}</label>
</p>
{*<p>
...
<label for="optin" style="font-size:8pt;">{l s='Recibir Ofertas especiales'}</label>
</p>*}
<p style="text-align:left;">
<input type="checkbox" name="sms" id="sms" value="1" autocomplete="off"/>
<label for="sms" style="font-size:8pt;">{l s='Recibir avisos y ofertas a tú celular'}</label>
</p>
</div>
<div style="display:table-cell; float:left; width: 48%; ">
...
{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'htmlall':'UTF-8'}" />{/if}
<div style="margin-left: 20%;margin-top: -86px;"><input type="submit" name="submitAccount" id="submitAccount" value="{l s='Register'}" /></div>
</p>
</div>
</div>
{/if}
  ...
  <!-- quest form --  formulario  modo invitado-->     
      {if isset($GUEST_FORM_ENABLED) &&$GUEST_FORM_ENABLED }   {include file="$tpl_dir./quest_form.tpl"}  {/if}
</div>
