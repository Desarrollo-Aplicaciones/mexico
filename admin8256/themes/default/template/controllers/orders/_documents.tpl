		...
		<td class="document_number">
			{if get_class($document) eq 'OrderInvoice'}
				{if isset($document->is_delivery)}
			   	...
			   	{else} {* &id_order_invoice={$document->id} *}
					<a target="_blank" href="{$link->getAdminLink('AdminPdf')|escape:'htmlall':'UTF-8'}&submitAction=generateInvoicePDF&id_order={$document->id_order}">
			   {/if}
			{elseif get_class($document) eq 'OrderSlip'}
