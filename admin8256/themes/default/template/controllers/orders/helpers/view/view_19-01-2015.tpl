...
{block name="override_tpl"}
    ...
    {if $orden_completa}
        ...
        {else}
            ...
            alert("La orden no esta Completa");
        {/if}
   ...
   $("#frm_pedido").submit(function(){
   
   var op_cod=$("#transporte_opcion").val();
   if(op_cod!=='')
   {
return true;
   }
...
<div class="bloc-command">
	...
	<div class="container-command">
		...
		<div style="width: 49%; float:left;">
			<!-- Change status form -->
			<form action="{$currentIndex}&vieworder&token={$smarty.get.token}" method="post" id="frm_pedido">
				

                                {if isset($opcion_transportador) && $opcion_transportador==TRUE}

                                    <div style="width: 400px;"> <b>
                                        Seleccione tipo COD </b>
                                            {if isset($opciones_transportador)}

                                                <select name="transporte_opcion" id="transporte_opcion">
                                                    <option value="">Seleccione COD</option>
                                                    {html_options options=$opciones_transportador }
                                                </select>

                                                {*$opciones_transportador|@var_dump*}
                                            {/if}
                                    </div>
                                    <br>
   ...
   <select id="id_order_state" {if $detener_estado} disabled="disabled" {/if}  name="id_order_state">
				{foreach from=$states item=state}
					...
					<table class="table" width="450px;" style="border-radius:0px;"cellspacing="0" cellpadding="0">
						...
						<tr id="total_shipping">
							...
							<td class="amount" align="right">
											{if $addresses.delivery->address1|upper  eq 'CALLE 129A NO. 56B - 23' ||
											 $addresses.delivery->address1|upper eq 'CALLE 129A NUMERO 56B - 23' ||
											 $addresses.delivery->address1|upper eq 'CALLE 129A # 56B - 23' ||
											 $addresses.delivery->address1|upper eq 'CARRERA 56B NO. 129A - 23' ||
											 $addresses.delivery->address1|upper eq 'CARRERA 56B NUMERO 129A - 23' ||
											 $addresses.delivery->address1|upper eq 'CARRERA 56B # 129A - 23' ||
											 $addresses.delivery->address1|upper eq 'CALLE 129A NO. 56B 23' ||
											 $addresses.delivery->address1|upper eq 'CALLE 129A NUMERO 56B 23' ||
											 $addresses.delivery->address1|upper eq 'CALLE 129A # 56B 23' ||
											 $addresses.delivery->address1|upper eq 'CARRERA 56B NO. 129A 23' ||
											 $addresses.delivery->address1|upper eq 'CARRERA 56B NUMERO 129A 23' ||
