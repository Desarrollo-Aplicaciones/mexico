...
<div class="bloc-command">
	<div class="button-command">
			{if (count($invoices_collection))}
				<a class="button" {if !$detener_estado} href="{$link->getAdminLink('AdminPdf')|escape:'htmlall':'UTF-8'}&submitAction=generateInvoicePDF&id_order={$order->id}"  target="_blank"{/if}>
	...
	<div class="container-command">
		...
		<div style="width: 49%; float:left;">
			...
			<form action="{$currentIndex}&vieworder&token={$smarty.get.token}" method="post">
				...
				{foreach from=$states item=state_0}
                                    
                                    {if $state_0['id_order_state']==3&&$state_0['id_order_state'] == $currentState->id}
                                       {assign var=disable_status value=false} 
                                        {break}                                       
                                    {/if}
				{/foreach}
                                ...
                                {if $disable_status}
									...
									<option disabled="disabled" selected="selected" value="3">Preparación en curso</option>
								</select>
                                    {else}
