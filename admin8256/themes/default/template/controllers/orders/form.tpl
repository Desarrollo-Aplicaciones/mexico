...
<script type="text/javascript">
	...
	var productosExpress = false;
		...
		$('#payment_module_name').change(function() {
			...
			// validación mostrar medio de pago 
			var lba	= $('#lba_medio_de_pago');
			var mop = $('#modo_de_pago');
			if($(this).val() == 'cashondelivery'){
				lba.css("display", "block");
				mop.css("display", "block");
			}else{
				lba.css("display", "none");
				mop.css("display", "none");
			}
		});
		$("#id_address_delivery").change(function() {
			var direccion = $("#id_address_delivery :selected").val();
			$("#id_address_invoice").val(direccion)
			updateAddresses();
			$("#express").removeAttr("checked");
		});
                $("#list_localidades").change(function() {
                    // alert("xd");
                    } );
		...
		$("#voucher").autocomplete('{$link->getAdminLink('AdminCartRules')|addslashes}', {
			...
			.result(function(event, data, formatted) {
				...
				if($('input:radio[name=opcselv]:checked').val() == 'medico'){
					add_medico(data.id_medico);
					$('#doctor').val(data.name);
			...
			$("#doctor").autocomplete('{$link->getAdminLink('AdminCartRules')|addslashes}', {
					...
					parse: function(data) {
						if (!data.found)
							$('#doctor_err').html('No se encontraron médicos').show();
						else
							$('#doctor_err').hide();
						var mytab = new Array();
						for (var i = 0; i < data.vouchers.length; i++)
		...
		$('.delete_product').live('click', function(e) {
			...
			$('#id_address_delivery').trigger('change');
	...
	function displaySummary(jsonSummary)
	{
		...
		jsonSummary.summary.total_discounts_tax_exc = jsonSummary.summary.total_discounts_tax_exc.replace(".","");
	...
	function updateQty(id_product, id_product_attribute, id_customization, qty)
	{
		$.ajax({
			...
			success : function(res)
			{
				...
				var errors = '';
				productosExpress = res.productExpress;
                                entregaNocturna = res.is_rules_entrega_nocturna;   
				$('#id_address_delivery').trigger('change');
	...
	function updateAddressesList(addresses, id_address_delivery, id_address_invoice)
	{
		...
		$.each(addresses, function() {
			...
			if(this.id_address == id_address_delivery)
				...
				{if $expressEnabled}
				if(this.express && id_address_delivery == this.id_address && productosExpress == true){
					address_delivery_detail = address_delivery_detail +
					'<br /><input type="checkbox" name="express" id="express" onchange="envio('+this.id_address+')" value="'+this.express+'"';
					if(typeof($("#express")) && $("#express").is(':checked') && productosExpress == true)
				    {
						address_delivery_detail = address_delivery_detail +' checked';
					}
					address_delivery_detail = address_delivery_detail +'> Envío Express';
				}
				{/if}
                             /* Validación envio nocturno (entrega nocturna) */   
                             {if isset($is_entrega_nocturna) && $is_entrega_nocturna}                   
			...
			if (this.is_rfc == 0){
				...
				rfc_detail = this.alias+' '+'<br />'+this.address1+'<br />'+this.postcode+' '+this.city+' '+this.country+'<br />'+this.dni;
		...
		if(rfc_detail.length > 0){
			$('#address_invoice_detail').html(rfc_detail);
		}
		else{
	...
	function validarCodPago(obj){
			...
			if(obj.id == 'cod_submit'){
				if($("#cod_pagar").val() == "" && $('#payment_module_name').val() == 'cashondelivery'){
		...
		if(error){
			...
			$('#modo_de_pago').append('<div id="cod_error" class="error">Selecciona una opción (Pagar con).</div>');
		}else{
				if($('#cod_error').length){
				$('#cod_error').remove();
			}
...
<form id="cod_form" action="{$link->getAdminLink('AdminOrders')|escape:'htmlall':'UTF-8'}&submitAdd{$table}=1" method="post" autocomplete="off">
<fieldset id="products_part" style="display:none;"><legend><img src="../img/t/AdminCatalog.gif" />{l s='Cart'}</legend>
	...
	<div>
		<table cellspacing="0" cellpadding="0" class="table width555" id="customer_cart">
				<colgroup>
					<col width="50px">
					<col width="">
					<col width="90px">
...
<fieldset id="vouchers_part" style="display:none;">
	...
	<p>
		...
		Cupón: <input type="radio" value="cupon" name="opcselv" id="opcselv1" checked="checked">
		Médico: <input type="radio" value="medico" name="opcselv" id="opcselv2">
...
<br />
<fieldset id="doctor_part" style="display:none;">
	<legend><img src="../img/t/AdminEmployees.gif" />Médicos</legend>
	<p>
...
<fieldset id="address_part" style="display:none;">
	...
	<div id="address_invoice">
		<h3>{l s='Invoice'}</h3>
		<select id="id_address_invoice" name="id_address_invoice" style="display:none;" disabled>
		</select>
...
<fieldset id="summary_part" style="display:none;">
	...
	<div class="order_message_right">
		...
		<label for="private_message">Mensaje Privado</label>
		<div class="margin-form">
			{literal}
				<textarea name="private_message" id="private_message" rows="3" cols="45" maxlength="5000"></textarea>
			{/literal}
		</div>
		<div class="margin-form">
		...
		<label id="lba_medio_de_pago">Pagar con</label>
		<div id="modo_de_pago" class="margin-form">
			<select id="cod_pagar" name="cod_pagar" onchange="validarCodPago(this);">
				<option value="">-Seleccionar pagar con-</option>
