...
<div class="margin-form">
	...
	<div id="cause">
		<select name="edit_supply_order" id="edit_supply_order" >
			<option value=""></option>
		{foreach $edit_supply_order as $cause}
			<option value="{$cause['id_causa']}">{$cause['causa']}</option>
		{/foreach}
		</select>
		<p class="preference_description">
			{l s='Seleccione las causas de la edición de la orden.'}
		</p>
	</div>
</div>
...
<script>
	function validaEstado(){
		if($('#id_supply_order_state').val() == 1){
			$('#cause').show();
			if ($('#edit_supply_order').val() == ""){
...
$('#edit_supply_order').change( function (){
	validaEstado();
});
</script>
{/block}
