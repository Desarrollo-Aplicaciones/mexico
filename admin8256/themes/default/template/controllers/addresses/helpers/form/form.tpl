...
{block name="field"}
	{if $input.type == 'text_customer'}
		...
		<div class="margin-form">
			<input type="checkbox" name="check_rfc" id="is_rfc" {if $fields_value['is_rfc']}checked{/if}/> <u style="color:red">sólo puede haber una dirección de facturación por cliente</u>
		</div>
		<script>
		$(document).ready(function(){
			if (!$('input#is_rfc').is(':checked')){
				$('#rfc').parent().prev('label').hide();
				$('#rfc').parent().hide();
			}
			$('#is_rfc').change(function(){
				$('#rfc').parent().prev('label').slideToggle();
				$('#rfc').parent().slideToggle();
				$('#alias').parent().prev('label').html('Nombre o razón social:');
			});
		});
		</script>
	{else}
