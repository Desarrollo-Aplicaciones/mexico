...
{block name="other_fieldsets"}
	...
	{if $edit_cause == NULL}
		...
		<p class="clear">O sube un archivo CSV con todos los productos relacionados de la orden, con el siguiente formato: product_reference, supplier_reference, precio_compra, cantidad, tasa_descuento, tasa_impuesto. Con separador (;).</p>
		<div id="test1"></div>
		<iframe id="testFrame" src="../paso1.html" frameBorder="0" height="63" width="600"></iframe>
					...
					>
						<thead>
							<tr class="nodrag nodrop">
								...
								{if $edit_cause == NULL}<th style="width: 40px">{l s='Delete'}</th>{/if}
							</tr>
						</thead>
						<tbody>
							{foreach $products_list AS $product}
								<tr style="height:50px;">
									...
									<td class="center">
										<input type="text" name="input_quantity_expected_{$product.id_product}_{$product.id_product_attribute}" value="{$product.quantity_expected|htmlentities}" {if $edit_cause != NULL}readonly{/if} size="5" />
									</td>
									<td class="center">
	...
	<script type="text/javascript">
		...
		function addProduct()
		{
			...
			$('#products_in_supply_order > tbody:last').append(
				...
				'<td class="center"><input type="text" name="input_tax_rate_'+product_infos.id+'" value="'+product_infos.tax+'" size="5" readonly />%</td>'+
	...
	<script>
	$('.margin-form').hide();
	$('.margin-form').prev().hide();
	$('[name="date_delivery_expected"]').parent().show();
	$('[name="date_delivery_expected"]').parent().prev().show();
	</script>
	{/if}
{/block}
