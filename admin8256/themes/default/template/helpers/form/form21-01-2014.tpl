...
{if isset($validateEmailCustomer) && $validateEmailCustomer == 'validateEmailCustomer'}

	<script type="text/javascript">
		//agrega evento onKeyUp al elemento email, para el llamado a la funcion AutoComplete
		$('#email').attr("onKeyUp", "AutoComplete(this, event)");

		//agrega evento onKeyPress al elemento email, para el llamado a la funcion ValidateArroba
		$('#email').attr("onKeyPress", "ValidateArroba()");

		//arreglo con sugerencia de email a autocompletar
		var autocompleteEmail = new Array("sinemail.com");

		//contador de arrobas
		var cantidadArroba = 0;

		//valida si existe un arroba en el textbox
		...
		//funcion para autocompletar email
		function AutoComplete(obj, evt) {

			//captura la tecla presionada
			var tecla = (obj.setSelectionRange) ? evt.which : evt.keyCode;

			//valida que la tecla sea un caracter
			if ((tecla < 32) || (tecla >= 33 && tecla <= 46) || (tecla >= 112 && tecla <= 123) || tecla == 225) {
				return;
			} else {
				//validar un unico @ en la caja de texto
				var texto = $('#email').val();
				var letraSelectionEmail = texto.charAt(texto.length - 1);
				if (letraSelectionEmail == "@"){
					if (cantidadArroba > 0){
						var textosinultimoarroba = texto.substring(0, texto.length-1);
						$('#email').val(textosinultimoarroba);
					} else {
						cantidadArroba++;
					}
				}
			...
			//se captura el valor de la caja de texto y se retiran espacios y/o caracteres especiales
			var txt = obj.value.replace(/;/gi, ",");
			elm = txt.split(",");
			txt = elm.pop();
			txt = txt.replace(/^\s*/, "");

			//se valida que la caja de texto contenga valor
			if (txt.length == 0) {
				return;
			}
			...
			//se recorre el arreglo con las sugerencias de email, y se marca la sugerencia deacuerdo a lo que se escribe en la caja de texto
			for (var i = 0; 
				i < autocompleteEmail.length; i++) {
				elm = autocompleteEmail[i].toString();
				if (txt.contains('@')) {
					arreglo = txt.split("@");
					if (arreglo[1].charAt(arreglo.length - 2) != ""){
						var cantidadLetrasTexto = arreglo[1].length;
						var stringSelection = arreglo[1].substring(0, cantidadLetrasTexto);
						var stringSelectionEmail = elm.substring(0, cantidadLetrasTexto);
						if (stringSelection == stringSelectionEmail){
							elm = elm.substr(cantidadLetrasTexto);
						} else {
							elm = "";
						}
					}
			...
			//selecciona de inicio a final, la sugerencia de email colocada en la caja de texto
			if (obj.createTextRange) {
