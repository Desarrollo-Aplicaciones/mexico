...
class AuthControllerCore extends FrontController
{
		...
		if ($this->ajax)
		{
			...
			die(Tools::jsonEncode($return));
                                        ...
                                        $sqlpais="SELECT ps_state.id_state, ps_state.name AS state 
                                            FROM ps_state 
                                            WHERE ps_state.id_country =  ".(int)Configuration::get('PS_COUNTRY_DEFAULT')." ORDER BY state ASC ;";
                                        ...
                                        $this->context->smarty->assign('pais',(int)Configuration::get('PS_COUNTRY_DEFAULT'));
	...
	protected function processSubmitAccount()
	{
             ...
             if(!is_null($this->context->customer->id ))
                {
                 Tools::redirect('index.php?controller=order&step=1&multi-shipping=0');   
                }
		...
		if (!Configuration::get('PS_REGISTRATION_PROCESS_TYPE') && !$this->ajax && !Tools::isSubmit('submitGuestAccount'))
		{
			if (!count($this->errors))
			{
                ...
                if ( isset($_POST['id'])) { $customer->id_type=$_POST['id'];}
                if ( isset($_POST['dni'])) { $customer->identification=$_POST['dni'];}
                if ( isset($_POST['sms'])) { $customer->sms=$_POST['sms'];}
                                /****** FIN ASIGNACION NUEVO CAMPOS *****/
		...
		{
                    //echo "<br>invitado<pre>";
                    

                    
                    /****** ASIGNACION NUEVOS CAMPOS *****/
                                if (isset($_POST['id'] )) { $customer->id_type= $_POST['id'] ; }
                                $customer->identification=$_POST['dni'];
                                if (isset($_POST['sms'] )) { $customer->sms=$_POST['sms']; }
                     /****** FIN ASIGNACION NUEVO CAMPOS *****/
			...
			if (!count($this->errors))
			{
				...
				else
				{
					...
					else
					{
						...
						$this->errors = array_unique(array_merge($this->errors, $address->validateController()));
						$address->city = Tools::getValue('city_id');
						//print_r($address);
    ...
    'id_city'=>(int)$city_id
		...
		if (count($this->errors))
		{
			...
			$this->context->smarty->assign('account_error', $this->errors);
                        ...
                        // valida si se debe mostrar  el fomulario de compra rapida
                        if(isset($_POST['display_guest_checkout'])&&isset($_POST['id_gender']))
                        {
                            if($_POST['display_guest_checkout']==1&&$_POST['id_gender']==0)
                            {
                                self::$smarty->assign('comprarapida',true );  
