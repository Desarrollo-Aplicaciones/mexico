...
protected function disableMediosP() {
    ...
    private function get_id_city_select_address() {
        try {
            ...
            $sql = 'SELECT adc.id_city
					...
					WHERE adc.id_address= ' . (int) (int)Context::getContext()->cart->id_address_delivery;
    ...
    protected function show_contra_entrega() {
    ...
    protected function block_medioP_sobre_costo() {
    ...
    protected function list_medios_de_pago() {
...
//$session_id=$this->context->smarty->tpl_vars['token']->value;
        ...
        self::$smarty->assign('deviceSessionId', $deviceSessionId);



        $query = "select mediosp.id_medio_de_pago,mediosp.Activo, IF (ISNULL(pepe.id_medio_de_pago),0,1) as inrule,mediosp.nombre, mediosp.type  
                  from ps_medios_de_pago mediosp LEFT JOIN 
                  ( SELECT mediospin.id_medio_de_pago, rules.id_ciudad FROM ps_medios_de_pago mediospin INNER JOIN ps_rules_mediosp_ciudades rules 
            ...
            if (count($results) > 0) {
                ...
                foreach ($results as $value) {
                    if ($value['Activo'] && $value['inrule']) { // Si el medio de pago esta en modo activo y la ciudad esta en la tabla rules, etonces se oculta para este medio de pago
                        ...
                        if ($value['type'] === '1') {
                            $list_medios_efectivo[] = array("active" => 0, 'name' => $value['nombre']);
                        }
                        ...
                        if ($value['type'] === '1') {
                            $list_medios_efectivo[] = array("active" => 1, 'name' => $value['nombre']);
                        }
...
// listado de medios de pago disponibles
        $str_list = '';
        $flag = true;
        ...
        /// Lista medios de pago en Efectivo  
        
        $str_list_efectivo = "";
        $flag2 = true;
        foreach ($list_medios_efectivo as $value) {

            if ($flag2) {
                $str_list_efectivo .=$value['name'] . '=' . $value['active'];
                $flag2 = FALSE;
            } else {
                $str_list_efectivo .='&' . $value['name'] . '=' . $value['active'];
            }
        }
        $output_efectivo = array();
        parse_str($str_list_efectivo, $output_efectivo);


        self::$smarty->assign('medios_de_pago', $output);
        self::$smarty->assign('medios_de_pago_efectivo', $output_efectivo);
