...
class ParentOrderControllerCore extends FrontController
{
		...
		if ($this->nbProducts)
		{
			if (CartRule::isFeatureActive())
			{
				if (Tools::isSubmit('submitAddDiscount'))
				{
					...
					$ProductsCartReductionCategory = $cart_for_rules->GetProductsCartReductionCategory( $ruleAdding );

					// si ProductsCartReductionCategory es true, no se agrega el cupon de descuento a la orden
					if ( $ProductsCartReductionCategory ) {
						$code = "";
						$this->errors[] = Tools::displayError('No se puede ingresar cupon, ya existen productos con descuento en el carrito');
					}
					elseif(count($rules_cart_added)>0 ) {						
						$this->errors[] = Tools::displayError('Ya ha ingresado un cupón a su pedido');
					} elseif (!($code = trim(Tools::getValue('discount_name')))){
					...
					{
						$valid_sel = Tools::getValue('type_voucher');
						
						if( $valid_sel == 'md' ) {							
							$iddoc = trim(Tools::getValue('doc_fnd'));
							$cartRule = new CartRule(CartRule::getIdByDoctor($iddoc));
						} elseif( $valid_sel == 'cupon' ) {
	...
	public function setMedia()
	{
		...
		if ((int)(Configuration::get('PS_BLOCK_CART_AJAX')) || Configuration::get('PS_ORDER_PROCESS_TYPE') == 1)
		{
			...
			$this->addJS(_THEME_JS_DIR_.'cart-summary.js', true);
		}
	}
