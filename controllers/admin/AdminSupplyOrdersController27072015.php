...
class AdminSupplyOrdersControllerCore extends AdminController
{
			...
			Tools::isSubmit('submitUpdatesupply_order'))
		{
			...
			$this->fields_form = array(
				...
				'input' => array(
					...
					array(
						'type' => 'text',
						'label' => $this->l('Factura de Proveedor:'),
						'name' => 'supplier_invoice',
						'size' => 50,
						'required' => false,
						'desc' => $this->l('número de factura entregada por el proveedor'),
					),
					array(
						'type' => 'select',
	...
	public function renderList()
	{
		...
		if (Tools::isSubmit('ordersFilter_id_supply_order')){
			$filter_supply_order['id_supply_order'] = Tools::getValue('ordersFilter_id_supply_order');
			$filter_supply_order['reference'] = Tools::getValue('ordersFilter_reference');
			$filter_supply_order['supplier'] = Tools::getValue('ordersFilter_s!name');
			$filter_supply_order['warehouse'] = Tools::getValue('ordersFilter_w!name');
			$filter_supply_order['state'] = Tools::getValue('ordersFilter_stl!name');
			$filter_supply_order['date_add'] = Tools::getValue('ordersFilter_a!date_add');
			$filter_supply_order['date_upd'] = Tools::getValue('ordersFilter_a!date_upd');
			$filter_supply_order['date_delivery_expected'] = Tools::getValue('ordersFilter_a!date_delivery_expected');
			$this->context->cookie->filter_supply_order = json_encode($filter_supply_order);
		}
		if (Tools::isSubmit('submitResetorders')){
			$filter_supply_order['id_supply_order'] = "";
			$filter_supply_order['reference'] = "" ;
			$filter_supply_order['supplier'] = "";
			$filter_supply_order['warehouse'] = "";
			$filter_supply_order['state'] = "";
			$filter_supply_order['date_add'] = "";
			$filter_supply_order['date_upd'] = "";
			$filter_supply_order['date_delivery_expected'] = "";
			$this->context->cookie->filter_supply_order = json_encode($filter_supply_order);
		}
		...
		if ($filtered['id_supply_order']){
			$this->_where .= ' AND a.id_supply_order LIKE (\'%'.$filtered['id_supply_order'].'%\')';
		}
		if ($filtered['reference']){
			$this->_where .= ' AND a.reference LIKE (\'%'.Tools::getValue('ordersFilter_reference').'%\')';
		}
		if ($filtered['supplier']){
			$this->_where .= ' AND s.name LIKE (\'%'.Tools::getValue('ordersFilter_s!name').'%\')';
		}
		if ($filtered['warehouse']){
			$this->_where .= ' AND w.name LIKE (\'%'.Tools::getValue('ordersFilter_w!name').'%\')';
		}
		if ($filtered['state']){
			$this->_where .= ' AND stl.name LIKE (\'%'.Tools::getValue('ordersFilter_stl!name').'%\')';
		}
		$filter_date_add = $filtered['date_add'];
		if ($filter_date_add[0]){
			$this->_where .= ' AND a.date_add > \''.$filter_date_add[0].' 00:00:00\'';
		}
		if ($filter_date_add[1]){
			$this->_where .= ' AND a.date_add < \''.$filter_date_add[1].' 23:59:59\'';
		}
		$filter_date_upd = $filtered['date_upd'];
		if ($filter_date_upd[0]){
			$this->_where .= ' AND a.date_upd > \''.$filter_date_upd[0].' 00:00:00\'';
		}
		if ($filter_date_upd[1]){
			$this->_where .= ' AND a.date_upd < \''.$filter_date_upd[1].' 23:59:59\'';
		}
		$filter_date_dde = $filtered['date_delivery_expected'];
		if ($filter_date_dde[0]){
			$this->_where .= ' AND a.date_delivery_expected > \''.$filter_date_dde[0].' 00:00:00\'';
		}
		if ($filter_date_dde[1]){
			$this->_where .= ' AND a.date_delivery_expected < \''.$filter_date_dde[1].' 23:59:59\'';
		}
	...
	public function initChangeStateContent()
	{
		...
		$causes = SupplyOrderState::getSupplyOrderCauses($supply_order->id_supply_order_state);
		...
		$helper->tpl_vars['edit_supply_order'] = $causes;
	...
	public function initUpdateSupplyOrderContent()
	{
		...
		if ($id_supply_order != null)
		{
			...
			if (Validate::isLoadedObject($supply_order))
			{
				...
				$this->tpl_form_vars['edit_cause'] = SupplyOrderState::getOrderCause($id_supply_order);
			}
		}
					...
					foreach ($states as $state)
					{
						...
						if ($id_state == $state['id_supply_order_state'])
						{
							...
							if(Tools::getValue('id_supply_order_state') == 2 && Tools::isSubmit('id_supply_order')){
								$supplier = Utilities::getSupplierById(Tools::getValue('id_supply_order'));
								if (strtolower($supplier['supplier_name']) == "nadro"){
									$path = _PS_ROOT_DIR_."/modules/ftpnadro/ftpnadro.dat";
									$error = Utilities::armarArreglo(Tools::getValue('id_supply_order'), $path);
									if (!$error){
										$error .= Utilities::conectarFTPNadro("Pedidos", "put", $path, "NP00LIST.".Tools::getValue('id_supply_order'));
									}
									if ($error){
							...
							if (!count($this->errors))
							{
								if ($supply_order->save())
								{
										...
										($old_state->receipt_state && $new_state->enclosed && !$new_state->receipt_state))
									{
										...
										if(Tools::getValue('id_supply_order_state') == 6)
										{
											if (Icrall::icrDel($id_supply_order))
												{
													$this->errors[] = Tools::displayError($this->l('Los ICR no pudieron ser desasociados.'));
												}
										}
									}
	...
	public function	ajaxProcessSearchProduct()
	{
		...
		$query->leftJoin('tax', 'tax', 'tax.id_tax = p.id_tax_rules_group');
