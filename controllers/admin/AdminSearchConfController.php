...
class AdminSearchConfControllerCore extends AdminController
{
	...
	public function __construct()
	{
		...
		$consulta = 'SELECT COUNT(*) as "0", SUM(product_shop.indexed) as "1" FROM '._DB_PREFIX_.'product p '.Shop::addSqlAssociation('product', 'p');
		if(Configuration::get('SEARCH_INDEX_ACTIVE') && Configuration::get('SEARCH_INDEX_ACTIVE') == 1){
			$consulta .= 'WHERE p.active = '.Configuration::get('SEARCH_INDEX_ACTIVE');
		}
		list($total, $indexed) = Db::getInstance()->getRow($consulta);
		...
		$this->fields_options = array(
			'indexation' => array(
				...
				'fields' =>	array(
					...
					'SEARCH_INDEX_ACTIVE' => array(
						'title' => $this->l('Omitir Productos Inactivos'),
						'validation' => 'isBool',
						'type' => 'bool',
						'cast' => 'intval',
						'desc' => $this->l('Habilitar para mostrar en los resultados únicamente los productos que se encuentren activos. Si se desactiva, buscará entre todos los productos (mayor tiempo de búsqueda).')
					)
				)
			),
			'search' => array(
