...
//                     $orden_salida =  new OrdenSuministroDetail();
//                     'orden_completa'=> $orden_salida->ordenCompleta($order->id)
//                          
//                       echo '<pre>:';
//                       print_r($estados);
//                       exit();
		...
		$this->tpl_view_vars = array(
			...
			'display_warehouse' => (int)Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT'),
                        'orden_completa' => $this->ordenCompleta($order->id),
                        'detener_estado'=>$estados['detener_estado']
		);
 ...
 if ($results = Db::getInstance()->ExecuteS($query)){
  ...
  return true;
   ...
   return false;     
    ...
    /*
     *  valida el estado de una orden de salida
     * 
     */

    public function statusOrder($OrderStates,$select)
                { 
 $array['detener_estado']= false; 
 
$query="SELECT config.`value`,o_state_l.`name`,str_destino.`name` status_name
FROM ps_configuration config
INNER JOIN (
select c_status.`name` from ps_conf_status c_status
INNER JOIN 
(select s_order.id_option_status from ps_status_options s_order
INNER JOIN ps_conf_status c_status ON (s_order.id_conf_status=c_status.id_conf_status)
WHERE c_status.`name` in
(select conf.`name` 
from ps_order_state_lang o_state_l
INNER JOIN ps_configuration conf ON(o_state_l.id_order_state=conf.`value`)
WHERE o_state_l.id_order_state= ".$select." and conf.`name` in(select name from ps_conf_status)) ) destino
ON (c_status.id_conf_status=destino.id_option_status) ) str_destino on (config.`name`=str_destino.`name`)
INNER JOIN ps_order_state_lang o_state_l ON (o_state_l.id_order_state=config.`value`)";
    ...
    {        
        ...
        { 
            ...
            foreach ($results as $row2)
            {
                ...
                // reglas pra no permitir el cambio de estado de la orden
               //  valida el estado actual
                
               
                if((int)$row['id_order_state']==(int)$select)
                {                    //echo 'XD:'+$this->ordenCompleta($this->id_object);                    exit();   
                    // si el estado actaul es "PS_OS_DELIVERED"  
                    //echo '<pre>';                        print_r($row2['status_name']);
                    if($row2['status_name']=='PS_OS_DELIVERED'&&!$this->ordenCompleta($this->id_object))
                    {                        
...
ps_transporte_opciones; ";
       ...
       if ($results = Db::getInstance()->ExecuteS($query)){
           ...
           foreach ($results as $valores) {
               ...
               $pepito[$valores['value']]=$valores['nombre'];
           }

           
        $this->context->smarty->assign(array('opciones_transportador'=>$pepito));
...
/*
 * Almacena la opcion de COD farmalisto
 */

public function opcionTransportista($order, $id_trasporte)
{
    $query="select id_order FROM ps_orders_transporte WHERE id_order=".$order->id;
     if ($results = Db::getInstance()->ExecuteS($query)){
        
       if($order->id===(int)$results[0]['id_order'])
       { // update
           $query="UPDATE ps_orders_transporte SET id_transporte_opcion=".$id_trasporte.", id_employee=".$this->context->employee->id." WHERE id_order=".$order->id;
          ...
          // Insert
$query="insert into ps_orders_transporte (id_order,id_transporte_opcion,extras,id_employee) VALUES(
".$order->id.", ".$id_trasporte.",NULL,".$this->context->employee->id."
)"; 
