	...
	public function ajaxProcessUpdateDeliveryOption()
	{
		if ($this->tabAccess['edit'] === '1')
		{
			if (Tools::getValue('express'))
				$this->context->cart->check_xps = true;
...
// 				//echo "<pre>";;echo "</pre>";
	...
	public function ajaxProcessAddVoucher()
	{
		if ($this->tabAccess['edit'] === '1')
		{
			...
			if(count($rules_cart_added)>0 ) {
				$errors[] = Tools::displayError('Ya ha ingresado un cupón a su pedido');
	...
	protected function getDeliveryOptionList()
	{
		...
		if (is_array(current($delivery_option_list))){
	...
	public function getProductxps(){
		$compare = array();
		$compare2 = array();
		$lista = "(";
		$resumen = $this->getCartSummary();
		foreach ($resumen[products] as $productos)
		{
