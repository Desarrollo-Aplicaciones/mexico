...
class AdminCustomersControllerCore extends AdminController
{
		...
		$this->fields_form = array(
			...
			'input' => array(
				...
				array(
					...
					'size' => 20,
					'required' => true,
					'hint' => $this->l('Forbidden characters:').' ñ-Ñ!<>,;?=+()@#"�{}_$%:'
				),
				array(
					'type' => 'text',
	...
	public function processAdd()
	{
		...
		$customer = new Customer();
 		$customer_identification = strval(Tools::getValue('identification'));	
 		//echo "<br>Admin_cus id:".$customer_identification;
 		if (!Validate::isIdentification($customer_identification)) {
 			//echo "<br>INVALIDO:".$customer_identification;
 			$this->errors[] = Tools::displayError('You have entered an invalid identity number').': '.$customer_identification;
 			$this->display = 'edit';
 			return $customer;
 		}
