...
class AdminProductsControllerCore extends AdminController
{
	...
	public function initToolbar()
	{
		...
		if ($this->display == 'edit' || $this->display == 'add')
		{
			if ($product = $this->loadObject(true))
                        { 
                            if($this->inBlackList) { $this->object->active=0; }
   ...
   {
                //   estodo del objeto product: $this->object->active
                // id empleado: $this->context->employee->id
                // id producto: $this->object->id
               // print_r($this->object->reference);
              //  echo '<br>AdminProductsControler -> 1815';
$total__rows = 0;
...
WHERE prod.reference ='" . $this->object->reference. "';";

        if ($results = Db::getInstance()->ExecuteS($query)) {
          
            foreach ($results as $row) {
                $total__rows = $row['total'];
            }
        }
        if($total__rows==0){
            return false;
        }else {
         $this->context->smarty->assign('inBlackList', true);
            return true;
        }
