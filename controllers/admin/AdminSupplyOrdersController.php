...
class AdminSupplyOrdersControllerCore extends AdminController
{
	...
	public function __construct()
	{
		...
		$this->fields_list = array(
			...
			'supplier_invoice' => array(
				'title' => $this->l('Factura Proveedor'),
				'width' => 100,
				'havingFilter' => true
			),
			'warehouse' => array(
	...
	public function renderForm()
	{
			...
			Tools::isSubmit('submitUpdatesupply_order'))
		{
			...
			$taxes = Tax::getTaxes();
			foreach ($taxes as $llave => $valores) {
				$taxes[$llave]['id_tax'] = round($valores['rate'], 0);
				$taxes[$llave]['rate'] = round($valores['rate'], 0).'%';
			}
			$taxes = array_merge(array('-'), $taxes);
			...
			$this->fields_form = array(
				...
				'input' => array(
					...
					array(
						'type' => 'text',
						'label' => $this->l('Costo base de Envío:'),
						'name' => 'shipping_base',
						'size' => 15,
						'required' => false,
						'desc' => $this->l('costo base de envío del proveedor'),
					),
					array(
						'type' => 'select',
						'label' => $this->l('Impuesto del envío:'),
						'name' => 'shipping_tax',
						'required' => true,
						'options' => array(
							'query' => $taxes,
						...
						'desc' => $this->l('Impuesto del envío si lo hay.')
					),
				),
				'submit' => array(
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('submitAddsupply_order') || Tools::isSubmit('submitAddsupply_orderAndStay'))
		{
			...
			if (!count($this->errors))
			{
				...
				if($id_supply_order != 0){
			 		$supply_order = new SupplyOrder($id_supply_order);
			 		$_POST['id_supply_order_state'] = 	$supply_order->id_supply_order_state;
			 	}
						...
						if ($id_state == $state['id_supply_order_state'])
						{
							...
							if(Tools::getValue('id_supply_order_state') == 2 && Tools::isSubmit('id_supply_order')){
								...
								if (strtolower($supplier['supplier_name']) == "nadro"){
									...
									if (!$error){
										$cuenta = Utilities::obtenerConsecutivo($supplier['id_supplier']);
										$error .= Utilities::conectarFTPNadro("Pedidos", "put", $path, "NP00LIST.".sprintf("%03s",$cuenta));
									}
									if ($error){
	...
	public function printExportIcons($id_supply_order, $tr)
	{
		...
		if ($supply_order_state->enclosed == true && $supply_order_state->receipt_state == true)
			$content .= '<a href="'.$this->context->link->getAdminLink('AdminSupplyOrders').'&amp;id_supply_order='.(int)$supply_order->id.'
						 ...
						 <a href="'.$this->context->link->getAdminLink('AdminPdf').'&submitAction=generateSupplyOrderReceiptPDF&id_supply_order='.(int)$supply_order->id.'" title='.$this->l('export FRT').'><img src="../img/os/71.png" /></a>';
		else
			$content .= '--';
		$content .= '</span>';
