...
class ValidateCore
{
	...
	*/
	...
	public static function isPrestaShopVersion($version)
	{
 	 ...
 	 /**
 	 * Check for Identification Number validity
 	 *
 	 * @param string $id identification to validate
 	 * @return boolean Validity is ok or not
 	 */
 	public static function isIdentification($id)
 	{
 		...
 		if ( !empty($id) && substr($id, 0, 1) != 0 && strlen($id) >= 5 && !preg_match('/1{4}|2{4}|3{4}|4{4}|5{4}|6{4}|7{4}|8{4}|9{4}|0{4}/', $id) && $cant <= 1 ) { // !empty($id) && strlen($id) >= 5 && !preg_match('(?!.*(.)\1{4})^[0-9]*$', $id) && !preg_match('/('.$consecutivos.')+/i', $id )
 			if ( ($id > 9999 && $id < 100000000) || ($id > 1000000000 && $id < 4099999999) ) {
 				return false;
 			}
 			else {
 				return true;
