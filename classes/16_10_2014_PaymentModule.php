...
//valida si tiene doble descuento tanto en la entrada de direccion como en cupon de envio adentro del back office
         ...
         if(isset($cart_rules[0]))
						...
						// echo '<br>';
									...
									if($address2 == 'CALLE 129A NO. 56B - 23' ||
											 $address2 == 'CALLE 129A NUMERO 56B - 23' ||
											 $address2 == 'CALLE 129A # 56B - 23'||
											 $address2 == 'CARRERA 56B NO. 129A - 23'||
											 $address2 == 'CARRERA 56B NUMERO 129A - 23'||
											 $address2 == 'CARRERA 56B # 129A - 23' ||
											 $address2 == 'CALLE 129A NO. 56B 23' ||
											 $address2 == 'CALLE 129A NUMERO 56B 23' ||
											 $address2 == 'CALLE 129A # 56B 23'||
											 $address2 == 'CARRERA 56B NO. 129A 23'||
											 $address2 == 'CARRERA 56B NUMERO 129A 23'||
										...
										// echo 'bn entro al primer ciclo<br>';
										
									
									if($opera == 1){
										if($address == 'BOGOTá, D.C.'){	if($addre == 'FARMALISTO'){}
										}
									}
									else{
											if($address == 'BOGOTá, D.C.'){	
												if($addre == 'FARMALISTO'){
													 if($cartruller == ""){
													
													$order->total_paid_real = $order->total_paid_tax_excl - $order->total_shipping;
													$order->total_discounts = $order->total_discounts_tax_excl+$order->total_shipping;
													$order->total_discounts_tax_excl=$order->total_discounts;
													$order->total_discounts_tax_incl=$order->total_discounts;
													$order->total_paid=$order->total_paid_real;
													$order->total_paid_tax_incl=$order->total_paid_real;
													$order->total_paid_tax_excl=$order->total_paid_real;
													}
												}
											}
								...
								if($address2 == 'CALLE 129A NO. 56B - 23' ||
										...
										if($address == 'BOGOTá, D.C.'){	
											...
											if($addre == 'FARMALISTO'){
												...
												if($cartruller != ""){
													...
													$porcentaje = $order->total_products_wt/100;
											// echo 'porcentaje '.$porcentaje.'<br>';
													$porcentaje = $porcentaje*$cartruller ;
											// echo 'porcentaje2 '.$porcentaje.'<br>';
											//$order->total_discounts_tax_incl = $order->total_paid_tax_incl - $order->total_shipping;
									...
									if(isset($opera) && $opera == 1){$order->total_paid_real = $order->total_paid;}
...
//Adicionar a la orden y al context el id_carrier del transportador con menor costo de envío
									...
									if($add_delivery != '') {
										...
										$sql_car = 'SELECT cac.precio_kilo, car.id_carrier, crp.delimiter2, car2.id_carrier AS carrier_cp, adtrcp.precio AS precio_cp
										...
										ORDER BY adtrcp.precio, cac.precio_kilo';
										...
										if ( $found_carrier == 1 ) {
											...
											if ($order->id_carrier != $carrier_sel ) 
											{
												$order->id_carrier = $carrier_sel;
												echo "<br>Car: ".$id_carrier = $carrier_sel;
											}

											foreach ($order->product_list as $key => $value) {
												foreach ($order->product_list[$key]['carrier_list'] as $key_arr => $value_arr) {

													$order->product_list[$key]['carrier_list'][$key_arr] = $carrier_sel;
...
//echo "order_status->logable: ".$order_status->logable." | cart_total_paid: ".number_format($cart_total_paid, 2)." | amount_paid: ".number_format($amount_paid, 2)." |  sumado: ".number_format($amount_paid + $val_envio_back, 2);
...
//exit;	
					//echo '<pre> amount_paid ';
				 //print_r($amount_paid);
				 //echo '<br> total_reduction_value_tex ';
				// print_r($order->total_shipping);
				 //echo '<br>este es el resultado esperado ';
				// print_r($val_envio_back);
					 ...
					 // print_r($amount_paid);			
						// 			echo '<br>';
													...
													$amount_paid = $amount_paid - $order->total_shipping;
						// 				echo '<pre>xxxx';
					 // print_r($amount_paid);			
						// 			echo '<br>';
												}
