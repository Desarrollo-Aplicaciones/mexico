	...
	class FacturaxionCore {
		...
		public function __construct( $test_mode = false ) {
			...
			if ( $test_mode == '1') {
				...
				$this->regimenfiscal = "REGIMEN GENERAL DE LEY DE LAS PERSONAS FISICAS CON ACTIVIDADES EMPRESARIALES";
...
/**
 * [simplexml_to_array para transformar un xml en un array]
 * @param  [string] $xml    [cadena xml a transformar]
 * @param  [array] &$array [variable que obtendrá el arreglo transformado del xml enviado]
 */
...
/**
 * [array_to_xml para transformar una estructura de array en un xml]
 * @param  [array] $template_info      [arreglo que se transformara en xml]
 * @param  [xml] &$xml_template_info [variable que obtendrá el valor resultante de la transformacion del array]
 */
...
/**
 * [LlavesRegistroTimbrado Para llamar los archivos necesarios para realizar el timbrado]
 * @param boolean $xml_sellar [string del xml a sellar]
 */
public function LlavesRegistroTimbrado( $xml_sellar = false ) {
...
/**
 * [sellarXML crear el sello del emisor con base en el xml enviado o genérico obtenido]
 * @param  [string] $cfdi               [string del xml a sellar por parte del emisor]
 * @param  [string] $numero_certificado [numero de certificado del emisor]
 * @param  [string] $archivo_cer        [nombre del archivo que tiene el certificado del emisor]
 * @param  [string] $archivo_pem        [nombre del archivo que tiene el certificado del emisor]
 */
function sellarXML($cfdi, $numero_certificado, $archivo_cer, $archivo_pem){
  ...
  if( !$xdoc->loadXML($cfdi) ){
  	$this->TimbradoNoOk( $this->id_order_t, "xml Invalido", 1, 0,$cfdi);
  } 
...
/**
 * [generarXML xml genérico para crear timbrado]
 * @param  [string] $rfc_emisor [rfc del emisor seleccionado ( pruebas - producción) ]
 * @return [string]             [string xml genérico]
 */
...
public static function replace_text(&$elemento1) {

    $buscar = array("á","é","í","ó","ú","Á","É","Í","Ó","Ú","ar6to67be_", "xmlns_", "xsi_", "a23r4e3r4eee_","'", '"', '<', '>', '¡', '¢', '£', '¤', '¥', '¦', '§', '¨', '©', 'ª', '«', '¬', '­', '®', '¯', '°', '±', '²', '³', '´', 'µ', '¶', '·', '¸', '¹', 'º', '»', '¼', '½', '¾', '¿', '×', '÷', 'Ð', 'ð', 'Þ', 'þ', 'Æ', 'æ', 'Œ', 'œ', 'Å', 'Ø', 'Ç', 'ç', 'ß', 'Ñ', 'ñ');

				$cambiar = array("&aacute;","&eacute;","&iacute;","&oacute;","&uacute;","&Aacute;","&Eacute;","&Iacute;","&Oacute;","&Uacute;","cfdi:", "xmlns:", "xsi:", "tfd:", "&apos;", '&quot;', '&lt;', '&gt;', '&iexcl;', '&cent;', '&pound;', '&curren;', '&yen;', '&brvbar;', '&sect;', '&uml;', '&copy;', '&ordf;', '&laquo;', '&not;', '&shy;', '&reg;', '&macr;', '&deg;', '&plusmn;', '&sup2', '&sup3;', '&acute;', '&micro;', '&para;', '&middot;', '&cedil;', '&sup1;', '&ordm;', '&raquo;', '&frac14;', '&frac12;', '&frac34;', '&iquest;', '&times;', '&divide;', '&ETH;', '&eth;', '&THORN;', '&thorn;', '&AElig;', '&aelig;', '&OElig;', '&oelig;', '&Aring;', '&Oslash;', '&Ccedil;', '&ccedil;', '&szlig;', '&Ntilde;', '&ntilde;');

	$elemento1 = str_replace( $buscar, $cambiar, $elemento1 );
	 ...
	 */
		...
		public function solicitud2( $metodo_pago, $cupon, $list_products, $invoice_address, $order_tot ) {
			...
			//echo "</br> </pre>";
			//exit();
			//echo "<div style='font-size:13px'> list_products: <pre style='font-size:13px'><font size='2'>"; //current_state
			//print_r($list_products);
			//echo "</hr> </pre>";
			
			
			$valor_timbrado = $this->RegistroTimbrado( $order_tot->id , 1);
			$this->id_order_t = $order_tot->id;
			...
			//exit;
			if ( ( !is_array($valor_timbrado) && $valor_timbrado == 0 ) && !file_exists($xml_previamente_generado)  )  {
			...
			//if ( !file_exists($xml_previamente_generado) ) {		
				...
				*/
				...
				foreach ($list_products as $key_prod => $value) {
					...
					$arr_xml_cargar_p['ar6to67be_Conceptos']['ar6to67be_Concepto'][$cant_prods]['@attributes']['descripcion'] = str_replace("<sup>FM</sup>  ", "", $list_products[$key_prod]['product_name']);
					...
					if ( number_format( $list_products[$key_prod]['tax_rate'] , 2, '.', '') != '0.00') {

						//echo "<br> si tax del ". /*number_format( */$list_products[$key_prod]['tax_rate'] /*, 2, '.', '')*/;
						...
						if ( isset( $cupon ) && $cupon != null && $cupon['reduction'] != '' ) {
							...
							if ( $cupon['tipo'] == 'porcentaje' ) {
								...
								$val_total_min_dto_mas_iva += //number_format( 
									( $list_products[$key_prod]['total_price_tax_excl'] - ( $list_products[$key_prod]['total_price_tax_excl'] * $cupon['reduction'] ) / 100 ) + $val_iva_prod_actual 
									/*, 2, '.', '')*/;
						...
						$val_iva_X_tax[$list_products[$key_prod]['tax_rate']] += $val_iva_prod_actual;
						...
						if ( isset( $cupon ) && $cupon != null && $cupon['reduction'] != '' ) {
							...
							//echo "<br> val total acumulado :".
							$val_total_min_dto_mas_iva += /*number_format(*/ $list_products[$key_prod]['total_price_tax_excl'] 
										/*, 2, '.', '')*/;
				...
				if ( isset( $cupon ) && $cupon != null && $cupon['reduction'] != '' ) {
					...
					if ( $cupon['tipo'] == 'porcentaje' ) {
						...
						$descuentop_aplicado = $subTotal_calculado - $cupon['reduction'];
					}
				...
				if ( $order_tot->total_shipping != '0.00' || $order_tot->total_shipping_tax_incl != '0.00' ) {
					...
					$val_no_iva_envio =  number_format( $order_tot->total_shipping / 1.16 ,3, '.', '');

					//echo "<br> - iva envio: ". 
					$val_iva_envio_act = /* number_format( ( */$order_tot->total_shipping  - $val_no_iva_envio/*) ,2, '.', '')*/;
					...
					$arr_xml_cargar_p['ar6to67be_Conceptos']['ar6to67be_Concepto'][$cant_prods]['@attributes']['cantidad'] = number_format( '1' ,2, '.', '');
					$arr_xml_cargar_p['ar6to67be_Conceptos']['ar6to67be_Concepto'][$cant_prods]['@attributes']['unidad'] = 'Pieza';
					$arr_xml_cargar_p['ar6to67be_Conceptos']['ar6to67be_Concepto'][$cant_prods]['@attributes']['valorUnitario'] = $val_no_iva_envio;
					$subTotal_calculado += $arr_xml_cargar_p['ar6to67be_Conceptos']['ar6to67be_Concepto'][$cant_prods]['@attributes']['importe'] = number_format( $val_no_iva_envio ,2, '.', '');
				...
				$arr_xml_cargar['@attributes']['fecha'] = date("Y-m-d")."T".date("H:i:s");//'2015-01-14T15:57:16';
				...
				$arr_xml_cargar['@attributes']['certificado'] = '';
				$arr_xml_cargar['@attributes']['sello'] = '';
				$arr_xml_cargar['@attributes']['subTotal'] = number_format($subTotal_calculado , 2, '.', '');
				$arr_xml_cargar['@attributes']['total'] = number_format($val_total_min_dto_mas_iva , 2, '.', '');

				if ($descuentop_aplicado != null ) {
					$arr_xml_cargar['@attributes']['descuento'] = number_format($descuentop_aplicado , 2, '.', '');
					$arr_xml_cargar['@attributes']['motivoDescuento'] = $cupon['description'];
				}
				...
				//$arr_xml_cargar['@attributes']['TipoCambio'] = '13.5';
					...
					$arr_xml_cargar['ar6to67be_Emisor']['@attributes']['nombre'] = $this->RFCEmisor_nombre;
						...
						$arr_xml_cargar['ar6to67be_Emisor']['ar6to67be_DomicilioFiscal']['@attributes']['calle'] = 'Tamaulipas';
						$arr_xml_cargar['ar6to67be_Emisor']['ar6to67be_DomicilioFiscal']['@attributes']['noExterior'] = '125';
						$arr_xml_cargar['ar6to67be_Emisor']['ar6to67be_DomicilioFiscal']['@attributes']['noInterior'] = '1';
						$arr_xml_cargar['ar6to67be_Emisor']['ar6to67be_DomicilioFiscal']['@attributes']['colonia'] = 'Roma';
						$arr_xml_cargar['ar6to67be_Emisor']['ar6to67be_DomicilioFiscal']['@attributes']['localidad'] = 'M&eacute;xico';
						$arr_xml_cargar['ar6to67be_Emisor']['ar6to67be_DomicilioFiscal']['@attributes']['referencia'] = 'Entre Le&oacute;n y Manzanillo';
						$arr_xml_cargar['ar6to67be_Emisor']['ar6to67be_DomicilioFiscal']['@attributes']['municipio'] = 'Cuauht&eacute;moc';
						...
						$arr_xml_cargar['ar6to67be_Emisor']['ar6to67be_DomicilioFiscal']['@attributes']['codigoPostal'] = '35143';
						...
						//$arr_xml_cargar['ar6to67be_Emisor']['ar6to67be_ExpedidoEn']['@attributes']['noInterior'] = '1';
						$arr_xml_cargar['ar6to67be_Emisor']['ar6to67be_ExpedidoEn']['@attributes']['colonia'] = 'Tacubaya';
						//$arr_xml_cargar['ar6to67be_Emisor']['ar6to67be_ExpedidoEn']['@attributes']['localidad'] = 'M&eacute;xico';
					...
					$arr_xml_cargar['ar6to67be_Receptor']['@attributes']['nombre'] = 'MOSTRADOR';       
						...
						if ( $invoice_address->firstname != '' && $invoice_address->firstname != null ) {

							$arr_xml_cargar['ar6to67be_Receptor']['@attributes']['nombre'] = htmlentities( $invoice_address->firstname ); //'Nombre del Receptor';

						}

						if ( $invoice_address->lastname != '' && $invoice_address->lastname != null ) {

							if ( $arr_xml_cargar['ar6to67be_Receptor']['@attributes']['nombre'] != '' ) {

								$arr_xml_cargar['ar6to67be_Receptor']['@attributes']['nombre'] .= " ".htmlentities( $invoice_address->lastname ); //'Nombre del Receptor';
						...
						if ( $invoice_address->address1 != '' && $invoice_address->address1 != null ) {
							$arr_xml_cargar['ar6to67be_Receptor']['ar6to67be_Domicilio']['@attributes']['calle'] = htmlentities( $invoice_address->address1 ); //'calle obligatoria';
						} else {
							$arr_xml_cargar['ar6to67be_Receptor']['ar6to67be_Domicilio']['@attributes']['calle'] = " No otorgada "; //'calle obligatoria';
						}

						if ( $invoice_address->colonia_name != '' && $invoice_address->colonia_name != null ) {
							$arr_xml_cargar['ar6to67be_Receptor']['ar6to67be_Domicilio']['@attributes']['colonia'] = htmlentities( $invoice_address->colonia_name );
						}

						if ( $invoice_address->address2 != '' && $invoice_address->address2 != null ) {
							$arr_xml_cargar['ar6to67be_Receptor']['ar6to67be_Domicilio']['@attributes']['referencia'] = htmlentities( $invoice_address->address2 );
						}

						if ( $invoice_address->country != '' && $invoice_address->country != null ) {
							$arr_xml_cargar['ar6to67be_Receptor']['ar6to67be_Domicilio']['@attributes']['pais'] = htmlentities( $invoice_address->country );
						}

						if ( $invoice_address->postcode != '' && $invoice_address->postcode != null ) {
							$arr_xml_cargar['ar6to67be_Receptor']['ar6to67be_Domicilio']['@attributes']['codigoPostal'] = htmlentities( $invoice_address->postcode );
						}
				...
				//array_walk_recursive($arr_xml_cargar, 'Facturaxion::replace_text');
			  ...
			  $xml214_1 = html_entity_decode( str_replace( $buscar, $cambiar, $xml1->saveXML() ) );
			   $this->TimbradoNoOk( $order_tot->id, "XML GENERADO ", 1, 0, $xml214_1 );
			  ...
			  //$contenido_fichero_1 = str_replace('"','&amp;quot;',$contenido_fichero_1);
				...
				//echo "<br> Sellado emisor:<br><textarea style='height: 574px;  width: 1849px;' style='font-size:13px'>"; echo html_entity_decode( str_replace( array("ar6to67be_", "xmlns_", "xsi_"), array("cfdi:", "xmlns:", "xsi:"), $xml2->saveXML() ) ); echo "</textarea><br><hr>";
 ...
 * @param  [int] $order_tot_id [id de la orden a cancelar el timbrado]
 * @param  [date] $order_tot_id [fecha de generación de la factura]
 * @return [array]            [retorna arreglo con los valores del timbrado]
 */
public function cancelacion( $order_tot_id, $order_invoice_date ) {
	...
	//$usuario="0000023492";
		...
		/**
		 * [TimbradoOk Registra en la bd los datos del timbrado]
		 * @param [int] $id_order      [id de la orden timbrada]
		 * @param array  $arr_timbre    [arreglo con los datos del timbrado]
		 * @param array  $arr_documento [arreglo con lso datos de la documentacion del timbrado ( rutaxml) ]
		 */
		...
		/**
		 * [TimbradoNoOk Guarda en la bd el error generado al momento de timbrar o cancelar el timbrado]
		 * @param [int] $id_order  [id de la orden en la que se generó el error]
		 * @param [string] $mensaje   [mensaje de error devuelto por el web service]
		 * @param [bool] $timbrado  [si fue al momento de timbrar]
		 * @param [bool] $cancelado [si fue al momento de cancelar]
		 */
		public function TimbradoNoOk( $id_order, $mensaje, $timbrado, $cancelado, $xml = null ) {
		...
		/**
		 * [RegistroTimbrado devuelve los datos del timbrado de la orden, si esta los posee en la bd]
		 * @param [type]  $id_order [id de la orden a validar]
		 * @param integer $timb     [si se busca timbrado de la orden]
		 * @param integer $canc     [si se busca orden cancelada]
		 */
		...
		/**
		 * [CanceladoOk Actualiza la BD si la orden se canceló correctamente]
		 * @param [int] $id_timbrado [id del registro del timbre de la orden la cual fue cancelada]
		 */
		...
		/**
		 * [CanceladoNoOk para almacenar el error generado al momento de cancelar la orden]
		 * @param [int]  $id_order  [id de la orden a la cual se le realizó el proceso de cancelación]
		 * @param [string]  $mensaje   [mensaje de error retornado por el web service]
		 * @param bool $timbrado  [si la orden es timbrada]
		 * @param bool $cancelado [si la orden es cancelada]
		 */
