...
class MailCore
{
		...
		$template_path = _PS_MAIL_DIR_, $die = false, $id_shop = null, $bcc = null)
	{
		...
		try {
			...
			$subject_email = '';
			if ($template == 'es/delivered'){ 
				$subject_email = 'Gracias por comprar con nosotros, califica nuestro servicio';			
			} else { 
				$subject_email = '['.Configuration::get('PS_SHOP_NAME', null, null, $id_shop).'] '.$subject;				
			}
			$message = new Swift_Message($subject_email);
