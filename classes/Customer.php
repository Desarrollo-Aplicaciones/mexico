...
class CustomerCore extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields' => array(
			...
			/***** NUEVOS CAMPOS ****/
                        ...
                        'identification' => 			array('type' => self::TYPE_STRING, 'validate' => 'isCleanHtml', 'copy_post' => false),
	...
	{
		$sql = 'SELECT DISTINCT a.*, cl.`name` AS country, s.name AS state, s.iso_code AS state_iso, cc.express_abajo AS express
				FROM `'._DB_PREFIX_.'address` a
				LEFT JOIN `'._DB_PREFIX_.'country` c ON (a.`id_country` = c.`id_country`)
				LEFT JOIN `'._DB_PREFIX_.'country_lang` cl ON (c.`id_country` = cl.`id_country`)
				LEFT JOIN `'._DB_PREFIX_.'state` s ON (s.`id_state` = a.`id_state`)
				'.(Context::getContext()->shop->getGroup()->share_order ? '' : Shop::addSqlAssociation('country', 'c')).'
				INNER JOIN `'._DB_PREFIX_.'address_city` AS ac ON (a.id_address=ac.id_address)
				INNER JOIN `'._DB_PREFIX_.'carrier_city` AS cc ON (ac.id_city=cc.id_city_des)
