			...
			if ($page_number <= $total_pages ) {
				...
				if ($results['result'] != NULL) {
					...
					foreach ($results['result'] as $value) {
						...
						$textocorto = '';
						//error_log($value['id_product'].'->'.strlen($value['name']));

						if( strlen($value['name']) > 55 ) {
							//error_log("0 val : ".$value['name']);
							$textocorto = mb_strcut( $value['name'], 0, 55 ).'...';
							/*error_log("1 val : ".$textocorto );
							$textocorto = preg_replace('/[^\s\p{L}]/u', '',$textocorto);
							error_log("2 val : ".$textocorto);
							$textocorto = $this->stripAccents( $textocorto );
							error_log("3 val : ".$textocorto);*/

						} else {
							$textocorto =  strtolower(strip_tags($value['name']));
						}
						//////--error_log($textocorto);
...
private function createOrder($method,$state) {
    ...
    public function add_image($files,$option){
    	...
    	if(!isset($files['name']) && !isset($files['type']) && !isset($files['tmp_name'])){
    		error_log('< | Debes enviar un archivo | >',0);
    	...
    	if (!isset($this->context->customer->id) || empty($this->context->customer->id)){
    		error_log('< | Debes iniciar sesión para agregar una foto | >',0);
...
//.'|'.end(explode('.',$files['name']));
    		...
    		if($option == 'profile'){
    			...
    			error_log('< | Guardando imagen de perfil | >',0);
    			return $this->context->customer->update();
    			
    		}
    		error_log('< | Guardando imagen de RX | >',0);
    	...
    	error_log('< | Error guardando archivo | >',0);
    	return FALSE;
    }
