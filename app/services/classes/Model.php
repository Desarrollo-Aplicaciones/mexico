...
class Model extends PaymentModule
{
        ...
        ORDER BY i.position  ASC";
            ...
            if ($results = Db::getInstance()->ExecuteS($query)) {
                foreach ($results as $value) {
                    $array_img[] = _PS_BASE_URL_ . __PS_BASE_URI__ . 'img/p/' . Image::getImgFolderStatic($value['id_image']) . $value['id_image'] . '-home_default.jpg';
                }
                ...
                if ($results['result'] != NULL) {
                    foreach ($results['result'] as $value) {
                        ...
                        if ($value['id_image'] != NULL) {
                            $img_url = _PS_BASE_URL_ . __PS_BASE_URI__ . 'img/p/' . Image::getImgFolderStatic($value['id_image']) . $value['id_image'] . '-home_default.jpg';
                            
                            $img_url_large = _PS_BASE_URL_ . __PS_BASE_URI__ . 'img/p/' . Image::getImgFolderStatic($value['id_image']) . $value['id_image'] . '-large_default.jpg';
                        } else {
                            $img_url = _PS_BASE_URL_ . __PS_BASE_URI__ . 'img/p/' . 'es-default-home_default.jpg';
                            
                            $img_url_large = _PS_BASE_URL_ . __PS_BASE_URI__ . 'img/p/' . 'es-default-large_default.jpg';
                        }
    ...
    /**
     *
     * @param $level_depth_min int
     *            nivel inferior de categorías
     * @param $level_depth_max int
     *            nivel superior de categorías
     * @param $con_activo bool
     *            si está en false traerá todas las categorías sin el filtro de active
     * @return array
     */
    ...
    public function cart($products_app, $id_customer, $id_address = 0, $discounts = NULL, $deleteDiscount = NULL, $msg = NULL, $id_cart = null, $clear = FALSE)
    {
        ...
        if (isset($this->context->cart) && ! empty($this->context->cart) && (isset($this->context->cookie->id_cart) && ! empty($this->context->cookie->id_cart))) {
            ...
            if (isset($discounts_return[0]) && $aplicar_cupon == 1 && ! isset($discounts_return[0]['success']) && ! $discounts_return[0]['success'] == true) {
                ...
                $discounts_return[] = array(
                    'success' => false,
                    'msg' => 'Cupon incorrecto, no aplicado'
                );
            }
            ...
            foreach ($cart_rules as $cart_rule) {
                if ((int) $cart_rule['gift_product']) {
                    foreach ($products as $key => &$product) {
                        if (empty($product['gift']) && (int) $product['id'] === (int) $cart_rule['gift_product'] && (int) $product['id_product_attribute'] === (int) $cart_rule['gift_product_attribute']) {
        ...
        if ($results = Db::getInstance()->ExecuteS($query)) {
            ...
            if ($results[0]['imgs'] != NULL) {
                foreach (explode(',', $results[0]['imgs']) as $value) {
                    $array_img[] = _PS_BASE_URL_ . __PS_BASE_URI__ . 'img/p/' . Image::getImgFolderStatic($value) . $value . '-large_default.jpg';
                }
            } else {
    ...
    SELECT ad.id_address,
      ...
      INNER JOIN ps_cod_postal AS cp ON (cp.codigo_postal = ad.postcode)
      INNER JOIN ps_cities_col cc ON (cc.id_city = cp.id_ciudad)
      WHERE ad.id_customer = " . (int) $id_customer . " 
      AND ad.is_rfc = 0 
      AND ad.deleted = 0" . ($id_address != NULL ? (" AND ad.id_address = " . (int) $id_address) : "") . " GROUP BY ad.id_address 
    ...
    public function setAddress($arg = array())
    {
        ...
        // return array('success' => false, 'message' => 'Dirección no es un objeto valido');
        if ($arg['id']) {
        ...
        /*
         * $id_address_rfc = $this->getAddressRFC($arg['id_customer']);
         * $address = NULL;
         * if ($id_address_rfc != 0) {
        ...
        $address->lastname = ! empty($customer->lastname) ? $customer->firstname : ' ';
        ...
        $address->dni = (! empty($arg['dni']) && Validate::isRFC($arg['dni'])) ? $arg['dni'] : '';
    ...
    public function pay($args)
    {
        ...
        foreach ($reglas_medios_pago as $medio_pago => $regla) {
            if ($medio_pago == $args['payment']['method'] && $regla == '0') {
                return array(
                    'success' => FALSE,
                    'message' => "Este medio de pago no es válido para la dirección ingresada por favor selecciona uno diferente"
                );
            }
        }
        ...
        if ($flg) {
            ...
            $this->state = PasarelaPagoCore::payOrder($data_payment);
            
            if ($this->state == Configuration::get('PS_OS_ERROR')) {
                $message = 'Ha ocurrido un error al realizar el pago, valida tus datos o intenta con otro medio de pago.';
                return array(
                    'success' => FALSE,
                    'message' => $message,
                    "erros" => PasarelaPagoCore::getLastErrorByCart($this->context->cart->id)
                );
            } else {
                return $this->createOrder($this->state);
            }
        } else { // pagos con cashondelivery
            $this->state = Configuration::get('PS_OS_PREPARATION');
            return $this->createOrder($this->state);
        }
    }
    ...
    /**
     */
    private function createOrder($state)
    {
        ...
        try {
            ...
            // Agregar a la sonda
            $conn = PasarelaPagoCore::GetDataConnect($this->method);
            $date = date("Y-m-d H:i:s");
            $interval = 0;
            if ($this->method == 'Tarjeta_credito') {
                $interval = 11;
            } else {
                $interval = 61;
            }
    ...
    private function response()
    {
        ...
        $obj = array();
        if ($this->id_order && ! count($this->errors) > 0) {
            $obj = array(
                'order' => $response,
                'pay_info' => $extra_vars,
                'success' => TRUE,
    ...
    public function add_image($files, $option)
    {
        ...
        if (! isset($this->context->customer->id) || empty($this->context->customer->id))
            return array(
                'ERROR' => 'Debes iniciar sesión para agregar la foto de tu perfil.'
            );
    ...
    public function password($email)
    {
        if (isset($email) && ! empty($email)) {
            if (! ($email = trim(Tools::getValue('email'))) || ! Validate::isEmail($email))
                ...
                if (! Validate::isLoadedObject($customer))
                    return array(
                        'success' => FALSE,
                        'message' => 'No hay una cuenta registrada con esta dirección de correo.'
                    );
                elseif (! $customer->active)
                    return array(
                        'success' => FALSE,
                        'message' => 'No puedes regenerar la contraseña para esta cuenta.'
                    );
                elseif ((strtotime($customer->last_passwd_gen . '+' . (int) ($min_time = Configuration::get('PS_PASSWD_TIME_FRONT')) . ' minutes') - time()) > 0)
                    return array(
                        'success' => FALSE,
                        'message' => 'Puede regenerar la contraseña cada ' . (int) $min_time . ' minutos '
                    );
                else {
                    $mail_params = array(
                        '{email}' => $customer->email,
                    ...
                    if (Mail::Send($this->context->language->id, 'password_query', Mail::l('Password query confirmation'), $mail_params, $customer->email, $customer->firstname . ' ' . $customer->lastname))
                        return array(
                            'success' => TRUE,
                            'message' => 'Enviamos enviado un mensaje al correo ' . $customer->email . ', sigue las instrucciones del mensaje para cambiar tu contraseña.'
                        );
                    else
                        return array(
                            'success' => FALSE,
        ...
        return array(
            'success' => FALSE,
            'message' => 'Debes colocar tu dirección de correo.'
        );
    }
