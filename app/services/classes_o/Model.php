...
*/
   		...
   		$array_prod[] = array(
   		                      'id' => (int) $value['id'],
   		                      'name' => strtolower($value['name']),
   		                      'shortname' => substr( strtolower(strip_tags($value['name'])) , 0 , 62 ),
	//'ref' => $value['reference'],
	//'desc' => strip_tags($value['description']),
	//'desc_short' => strip_tags($value['description_short']),
   		                      'price' => number_format( $value['price'] ,2, '.', ''),
   		                      'bprice' => number_format( $value['bprice'] ,2, '.', ''),
   		                      'img' => $this->getImageProduct($value['id'])[0],
   		                      'img_lab' => _PS_BASE_URL_.'/img/tmp/manufacturer_mini_74_1.jpg'
   		                      );
	...
	//$payment = new PaymentWs();
       ...
       $order_state = Db::getInstance()->getValue("SELECT  `name` FROM ps_order_state_lang WHERE id_order_state = ". (int) $state);
        ...
        if (!count($this->errors) > 0) {
        	$obj = array( 'order' => $response, 'success' => TRUE,'message'=>'Orden creada satisfactoriamente.', 'order_state' => $order_state);
        } else {
        	$obj = array('response' => $response, 'success' => FALSE,'message' => $this->errors['message'] , 'order_state' => $order_state);
        }
        return $obj;
    }

    public function add_formula($str_img,$option){

    	$re = "/^data:image\\/[\\D]{3,14};base64,.*$/"; 
    	if(!preg_match($re, $str_img))
    		return array('ERROR'=>'La cadena (str_img) no contiene un formato reconocido de imagen.');
