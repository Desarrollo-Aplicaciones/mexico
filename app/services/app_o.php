...
public function addImg(){
	...
	$model = new Model();
	$this->response(json_encode( $model->add_formula($str_img,$option)),200);
}

public function recoveryCart() {

	$param['id_cart'] = $this->_request['id_cart'];
	
	$model = new Model();
	return $this->response($this->json($model->cart(null,null,null,null,null,null, $param['id_cart']),200));
	exit( json_encode( $totales ) );
