...
class SevenEleven extends PayUControllerWS {
    ...
    public function process() {
        ...
        if (isset($_POST['pagar'])) {
            ...
            $fecha7ELEVEN = date('Y-m-d', $nuevafecha) . 'T' . date('h:i:s', $nuevafecha);
            $keysPayu = $conf->keys();

// Script Json payuLatam (7ELEVEN)   
...
';
            ...
            if ($response['code'] === 'ERROR') {
                ...
                $conf->failed_transaction(0.0, $customer->id, $data, $response, '7ELEVEN', $response['code'], $this->context->cart->id, $id_address);
            ...
            elseif ($response['code'] === 'SUCCESS' && $response['transactionResponse']['state'] === 'PENDING' && $response['transactionResponse']['responseMessage'] != 'ERROR_CONVERTING_TRANSACTION_AMOUNTS') {
                   $extra_vars = array('method' => '7ELEVEN',
                    ...
                    $this->createPendingOrder($extra_vars, '7ELEVEN', 'El sistema esta en espera de la confirmación de la pasarela de pago.');
                    ...
                    $conf->pago_payu($order['id_order'], $customer->id, $data, $response, '7ELEVEN', $extras, $id_cart, $id_address);
                ...
                $conf->failed_transaction(0.0, $customer->id, $data, $response, '7ELEVEN', $response['transactionResponse']['state'], $id_cart, $id_address);
