...
$(document).ready(function() {
    ...
    $('"input[name=opc_ini]:radio"').change(function() {
        ...
        if(this.value != "authentication") {       
            $( "#authenticationlist" ).hide();
        } else { 
            $('#authenticationlist').show( "slow" );
        }

        if(this.value != "maildelivered") {       
            $( "#maildeliveredlist" ).hide();
        } else { 
            $('#maildeliveredlist').show( "slow" );
        }
  ...
  $("#formpublicidad").validate({
            ...
            rules: {
                ...
                authenticationsel : {
                    required: { 
                        depends: function(element) {
                         return $('input[name=opc_ini]:checked').val() == 'authentication';
                    }
                }},
                ...
                maildeliveredsel : {
                    required: { 
                        depends: function(element) {
                         return $('input[name=opc_ini]:checked').val() == 'maildelivered';
                    }
                }},
            },
            messages: {
                ...
                authenticationsel : { required:" Seleccione una ubicación de inicio de sesion " },
                maildeliveredsel : { required:" Seleccione una ubicación de correo orden entregada " },
...
<form name="formpublicidad" action="" id="formpublicidad"  method="post">
    <fieldset><legend></legend>
        ...
        <div class="opciones"> <input type="radio" name="opc_ini" value="authentication"> Página de Inicio de Sesión</div>
        <div class="opciones"> <input type="radio" name="opc_ini" value="maildelivered"> Correo Orden Entregada</div>
...
<label id="opc_ini-error" class="error" style="display: none;" for="opc_ini"> </label>
        ...
        <div id="authenticationlist" style="display: none; float: left; width: 100%; margin: 20px;">  Ubicación : 
            <select style="width: auto; " name="authenticationsel" id="authenticationsel" OnChange="sendvalue(this);">    {html_options options=$authentication} </select>
        </div>

        <div id="maildeliveredlist" style="display: none; float: left; width: 100%; margin: 20px;">  Ubicación : 
            <select style="width: auto; " name="maildeliveredsel" id="maildeliveredsel" OnChange="sendvalue(this);">    {html_options options=$maildelivered} </select>
        </div>
