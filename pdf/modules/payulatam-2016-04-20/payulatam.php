...
//PAYU_WAITING_PAYMENT
    ...
    public function getContent() {
        ...
        if (isset($_POST) && isset($_POST['submitPayU'])) {
            ...
            if ( isset($_POST['modulo_pago']) && $_POST['modulo_pago'] != null && $_POST['modulo_pago'] != '' 
                && Configuration::updateValue('PASARELA_PAGO_SELECCIONADA', Tools::safeOutput(Tools::getValue('modulo_pago'))) ) {
                $html .= $this->_displayValidation();
            } else {
                $this->_postErrors[] = 'No se pudo actualizar la pasarela seleccionada';
                $html .= $this->_displayErrors();
            }
        }
    ...
    private function _displayAdminTpl() {
        $this->context->smarty->assign('pasarela_seleccionada', PasarelaPagoCore::GetSelected());
    ...
    public function hookPayment($params) {
        ...
        // si la pasarela es openPay
        if(Configuration::get('PASARELA_PAGO_SELECCIONADA') === 'openpay'){
           $kesy = $this->get_keys_pasarela(); 
            ...
            $year = date('Y-m-j');
            $year_select='<select id="año" onchange="cambiaFecha()" class="select-fecha-tarjetas" name="Year" name="year" data-openpay-card="expiration_year">
        ...
        $this->context->smarty->assign('pasarela_de_pago',Configuration::get('PASARELA_PAGO_SELECCIONADA'));
...
WHERE banco.codigo= " . (int) $codigo_banco . " AND rule.active=1 AND '" . $date . "' BETWEEN rule.date_from  AND rule.date_to;";
    ...
    public function get_keys_pasarela(){
        ...
        if(Configuration::get('PAYU_DEMO') === 'yes'){
           $mode=0;
        }
        ...
        $sql="SELECT datos.mercid,datos.apikey_privatekey ,datos.pseco_publickey, datos.apilogin_id, datos.accountid FROM "._DB_PREFIX_."datos_pasarelas_pago datos INNER JOIN "._DB_PREFIX_."medios_de_pago mediosp ON (datos.medios_pago = mediosp.id_medio_de_pago)
                INNER JOIN "._DB_PREFIX_."pasarelas_pago pasarela ON(datos.id_pasarela = pasarela.id_pasarela)
                    WHERE pasarela.nombre ='".Configuration::get('PASARELA_PAGO_SELECCIONADA')."' AND datos.produccion = ".$mode.";";
