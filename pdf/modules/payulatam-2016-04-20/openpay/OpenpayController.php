...
public function __construct($id = null,$api_key = null){
	if(empty($id) || empty($api_key)){
		...
		//$this->open_pay->setProductionMode(true);
...
{ 
	...
	$customerData = array(
     ...
     'address' => array(
         'line1' => substr($address->address1,0,199),
         'line2' => substr($address->address2,0,49),
         'line3' => substr($address->other,0,49),
...
$chargeRequest = array(
    					...
    					'metadata' => array(
      										'address1' 		=> substr($address->address1,0,30),
      										'address2' 		=> substr($address->address2,0,30),
      										'address3' 		=> substr($address->other,0,30),
      										'phone'			=> substr($address->phone_mobile .' '.$address->phone,0,30),
      										'fecha_compra' 	=> date("Y-m-d H:i:s"), 
      										'total' 		=> $this->context->cart->getOrderTotal(),
      										'descuento' 	=> $this->context->cart->getOrderTotal( Cart::ONLY_DISCOUNTS ),
      										'iva' 			=> $this->context->cart->getOrderTotals()['total_iva'],
      										'nombre' 		=> substr($customer->firstname.' '.$customer->lastname,0,30),
