...
if($response['code'] === 'ERROR')
{
 ...
 $conf->error_payu($id_order, $customer->id, $data, $response, 'BANCOMER', $response['transactionResponse']['state'], $this->context->cart->id, $id_address); 
 $messsage='<h1>¡Error!, Transaccion Rechazada</h1><br>'.$response['error'];    
        ...
        elseif ($response['code'] === 'SUCCESS' && $response['transactionResponse']['state'] === 'PENDING' && $response['transactionResponse']['responseMessage'] != 'ERROR_CONVERTING_TRANSACTION_AMOUNTS') {
            ...
            $convenio = array('label' => 'Convenio N° CIE:','value'=>'0720917' );
            $extra_vars =  array('method'=>'BANCOMER',
                            ...
                            $this->createPendingOrder($extra_vars, 'BANCOMER', 'El sistema esta en espera de la confirmación de la pasarela de pago.', 'PAYU_WAITING_PAYMENT');
                            ...
                            $conf->pago_payu($order['id_order'], $customer->id, $data, $response, 'BANCOMER', $extras.';'.json_encode($convenio), $id_cart,$id_address);
