...
//PAYU_WAITING_PAYMENT
	...
	private function _postProcessCredentials()
	{
		...
		Configuration::updateValue('PAYU_DEMO', pSQL(Tools::getValue('demo')));
                echo'Dni Number: '.getValue('dninumber');
	}
		...
		if (Validate::isLoadedObject($this->context->cart))
		{
			...
			foreach ($id_orders as $val)
			{
				...
				if ($this->context->cart->getOrderTotal() != $amount)
					Logger::AddLog('[Payulatam] The shopping card '.(int)$idCart.' doesn\'t have the correct amount expected during payment validation.'.Configuration::get('PAYU_API_KEY').'~'.Tools::safeOutput(Configuration::get('PAYU_MERCHANT_ID')).'~payU_'.Configuration::get('PS_SHOP_NAME').'_'.(int)$this->context->cart->id.'~'.number_format((float)$this->context->cart->getOrderTotal(),2,'.','').'~'.$currency->iso_code.'~'.$statePol."---".$amount, 2, null, null, null, true);
				else
				{
					$currency = new Currency((int)$this->context->cart->id_currency);
					if ($token == md5(Configuration::get('PAYU_API_KEY').'~'.Tools::safeOutput(Configuration::get('PAYU_MERCHANT_ID')).'~payU_'.Configuration::get('PS_SHOP_NAME').'_'.(int)$this->context->cart->id.'~'.number_format((float)$this->context->cart->getOrderTotal(),2,'.','').'~'.$currency->iso_code.'~'.$statePol) ||  $token == md5(Configuration::get('PAYU_API_KEY').'~'.Tools::safeOutput(Configuration::get('PAYU_MERCHANT_ID')).'~payU_'.Configuration::get('PS_SHOP_NAME').'_'.(int)$this->context->cart->id.'~'.number_format((float)$this->context->cart->getOrderTotal(),1,'.','').'~'.$currency->iso_code.'~'.$statePol) ||  $token == md5(Configuration::get('PAYU_API_KEY').'~'.Tools::safeOutput(Configuration::get('PAYU_MERCHANT_ID')).'~payU_'.Configuration::get('PS_SHOP_NAME').'_'.(int)$this->context->cart->id.'~'.number_format((float)$this->context->cart->getOrderTotal(),0,'.','').'~'.$currency->iso_code.'~'.$statePol)) // CUANDO SE ENVIAN # ENTEROS EN EL PAGO A PAYU, ESTE RETORNA 1 DECIMAL, CUANDO SE ENVIAN DECIMALES, PAYU RETORNA 2 DECIMALES. SE VALIDA TAMBIEN SIN DECIMALES EVG GPB
					{
						if ($statePol == 7)
						...
						Logger::AddLog('[PayU] The shopping card '.(int)$idCart.' has an incorrect token given from payU during payment validation.'.Configuration::get('PAYU_API_KEY').'~'.Tools::safeOutput(Configuration::get('PAYU_MERCHANT_ID')).'~payU_'.Configuration::get('PS_SHOP_NAME').'_'.(int)$this->context->cart->id.'~'.number_format((float)$this->context->cart->getOrderTotal(),2,'.','').'~'.$currency->iso_code.'~'.$statePol."--".number_format((float)$this->context->cart->getOrderTotal(),1,'.','')."--".$token, 2, null, null, null, true);
				}
				if (_PS_VERSION_ >= 1.5)
				{
