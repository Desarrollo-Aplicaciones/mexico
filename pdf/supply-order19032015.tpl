...
<style>
	...
	.taxes{width:100%;text-align:right;font-size:8pt;}
...
<div class="content">
	<table class="container">
		<tr>
			...
			<th style="width: 5%;">{* l s='Discount Rate' pdf='true' *} Des % </th>
			<th style="width: 10%;">{* l s='Total TE' pdf='true'}{l s='After discount' pdf='true' *} Valor con descuento</th>
		...
		<tr {if $i%2}class="darks"{/if}>
			<td style="text-align: left;"><!--{$i++}-->{$supply_order_detail->supplier_reference}</td>
			...
			{$descuento = $neto - ($neto * ($supply_order_detail->discount_rate/100))}
			{$sumD = $sumD + $descuento}</td>
			<td style="text-align: right;">{$currency->prefix}{$descuento}{$currency->suffix}</td>
			<td style="text-align: center;">{$supply_order_detail->tax_rate}
			{$ntax = $descuento + ($neto * ($supply_order_detail->tax_rate/100))}
			{$sumT = $sumT + $ntax}
			</td>
...
<div class="taxes">
	<table>
		<tr>
			...
			<th>{* l s='Tax Value' pdf='true' *}Valor Total</th>
		</tr>
...
<span>{* l s='Summary:' pdf='true' *}Resumen</span>
<!-- TOTAL -->
<div class="taxes">
	<table>
		<tr>
		...
		<tr>
			<td class="darks">{* l s='Total TE' pdf='true'} <br /> {l s='(Discount included)' pdf='true' *}
			Valor con Descuento
			</td>
