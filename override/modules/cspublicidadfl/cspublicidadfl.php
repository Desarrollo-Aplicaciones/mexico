...
/**
	...
	public function getContent() {
		...
		if (isset($_POST['opc_ini']) && $_POST['opc_ini'] != '' && isset($_POST[$_POST['opc_ini'].'sel']) && $_POST[$_POST['opc_ini'].'sel'] != '' 
			...
			if ($load_config = Db::getInstance()->ExecuteS($query_config)) {
				...
				$this->context->smarty->assign(array('nombre_list' => array('' =>' -- Seleccione -- ', 'index','product','category','search','authentication')));
				$this->context->smarty->assign(array('nombre_list' => array('' =>' -- Seleccione -- ', 'index','product','category','search','maildelivered')));
				...
				$this->context->smarty->assign(array('tipo_listb' => array(''=> ' -- Seleccione -- ', 'banner'=>'banner')));
	...
	public function displayForm()
	{
		...
		$list_paginas_banner = array('search' => 'search', 'category' => 'category', 'product' => 'product', 'index' => 'index', 'authentication' => 'authentication', 'maildelivered' => 'maildelivered' ); 		
	...
	public function loadConfigPublicidadFull() {
		...
		if ($load_config = Db::getInstance()->ExecuteS($query_config)) {
			...
			foreach ($load_config as $valores) {
           	...
           	$this->context->smarty->assign(array('authentication'=> array_merge((array)$seleccion_vacia, (array)$configfull['authentication']) ));
           	$this->context->smarty->assign(array('maildelivered'=> array_merge((array)$seleccion_vacia, (array)$configfull['maildelivered']) ));
	...
	public function hookloginizq($params)
	{
		if ( isset($this->context->controller->php_self) && $this->context->controller->php_self == 'authentication') {
			...
			if($config_show != false) {
				...
				if ( $this->cargapublicidad('authentication', $config_show, $ubicacion) ) {
					$this->context->smarty->assign('authentication', true);
					return $this->display(__FILE__, '/tpl/loginbanner.tpl');
				} else {
					return false;
				}
	...
	public function hookmaildeliveredinf($params)
	{
		if ( isset($this->context->controller->php_self) && $this->context->controller->php_self == 'maildelivered') {
			$ubicacion = "inferior";
			$config_show = $this->loadConfigPublicidad($ubicacion);

			if($config_show != false) {

				if ( $this->cargapublicidad('maildelivered', $config_show, $ubicacion) ) {
					$this->context->smarty->assign('maildelivered', true);
					return $this->display(__FILE__, '/tpl/maildeliveredbanner.tpl');
				} else {
					return false;
				}
