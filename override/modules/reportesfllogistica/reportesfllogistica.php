...
class reportesfllogistica extends Module
{
	private $_html = '';
	private $output = '';
	...
	function __construct()
	{
		...
		$this->displayName = $this->l('Reportes Logistica Farmalisto');
		$this->description = $this->l('Listado de reportes logistica Farmalisto.');
	...
	{	
		...
		if (!$id_tab = Tab::getIdFromClassName('AdminReportesfllogistica'))
		{
			$tab = new Tab();
			$tab->class_name = 'AdminReportesfllogistica';
			$tab->module = 'reportesfllogistica';
			$tab->id_parent = (int)Tab::getIdFromClassName('AdminParentStats'); //aparecerá al final del menú estadisticas

			foreach (Language::getLanguages(false) as $lang)
			$tab->name[(int)$lang['id_lang']] = 'Reportes Logistica';
			if (!$tab->save())
				return $this->_abortInstall($this->l('Imposible crear la pestaña de nuevo'));
		}

		$this->_clearCache('reportesfllogistica.tpl');
	...
	public function uninstall()
	{
		...
		$tab = new Tab();
		$id_tab = $tab->getIdFromClassName("AdminReportesfllogistica");
		$tab->id = $id_tab;
		if ( !$tab->delete() ) {
	...
	public function getContent() {
        ...
        if (Tools::getValue('opc_ini')) {
    		...
    		switch ($primeraopc) {
    			case 'consped':
    				// $this->output.= "<div style='width:100%; float: left;'> Crear visitador médico </div>";
    			...
    			case 'icrsumi':
    				//$this->output.= "<br> Modificar cupón médico"; ord_consul
					if (Tools::getValue('executeconsul')) {
						if (Tools::getValue('executeconsul') == "Continuar") {	    							
							if( Tools::getValue('ord_consul') != '' && Tools::getValue('consped_f_ini2') != '' && Tools::getValue('consped_f_fin2') != '' ) {

								$date_format = 'Y-m-d';
								$input1 = Tools::getValue('consped_f_ini2');
								$input2 = Tools::getValue('consped_f_fin2');
								$input1 = trim($input1);
								$input2 = trim($input2);
								$time1 = strtotime($input1);
								$time2 = strtotime($input2);
								...
								if ( date($date_format, $time1) == $input1 && date($date_format, $time2) == $input2) {
    								$this->displayConfirmation($this->l('Reporte generado correctamente.'));
    								return $this->displayXlsDown($primeraopc."&orden=".Tools::getValue('ord_consul')."&f_ini=".Tools::getValue('consped_f_ini2')."&f_fin=".Tools::getValue('consped_f_fin2'));
    							} else {
									$this->output .= $this->adminDisplayWarning($this->l('Formato de fecha erróneo o fecha incorrecta.'));
									return $this->output . $this->displayForm();
								}
    ...
    public function displayXlsDown($opcion) {
    	header ("Location: ../modules/reportesfllogistica/xlsdownload.php?opc_sel=".$opcion);
