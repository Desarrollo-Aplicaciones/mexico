...
console.log(response);
   ...
   return 0;
    ...
    function expressEnabled()
    {
		if($("#express").is(':checked'))
	    {
	        express(1);
	        $("#xpsEstado").removeAttr("disabled");
	        $("#xps_options").show(500);
	    }
		else
		{
			express();
			$("#xpsEstado").attr("disabled", true);
			$("#xps_options").hide(500);
		}
    }
    function express(value)
    {
        $.ajax({
            type: "post",
            url: "../modules/TransporteCiudades/envioExpress.php",
            data: {
                "express": 'update', "status": value
            },
            success: function(response){
                if(response){
                    alert(response)
    ...
    function expressEstado()
    {
    	var ruta = "../ajax_formulario_cities.php"; 
		var valor = $("#xpsEstado").val();
        $.ajax({
            type: "post",
            url: ruta,
            data: {
            ...
            success: function(response){
                ...
                $('#xpsCiudad').html('<option value="0" selected>--Seleccione--</option>' + json.results);
                $("#xpsCiudad").removeAttr("disabled");
                $("#xpsAbajo").attr("disabled", "true");
                $("#xpsArriba").attr("disabled", "true");
                $("#saveXps").removeAttr("href");
                $("#saveXps").attr("style","opacity: 0.4;");
                $("#xpsAbajo").val("");
                $("#xpsArriba").val(""); 
            },
            error: function(err){
    ...
    function expressCiudad()
    {
        $.ajax({
            type: "post",
            url: "../modules/TransporteCiudades/envioExpress.php",
            data: {
                "express": 'checkCiudad', "value": $("#xpsCiudad").val()
            },
            success: function(response){
                if(response){
                    var values = response.split("::")
                    $("#xpsAbajo").removeAttr("disabled");
                    $("#xpsArriba").removeAttr("disabled");
                    $("#saveXps").attr("style","opacity: 1;");
                    $("#saveXps").attr("href","#");
                    $("#xpsAbajo").val(values[0]);
                    $("#xpsArriba").val(values[1]); 
                }}
        });
    }
    function guardarXps()
    {
        var xpsAbajo = $("#xpsAbajo").val(), xpsArriba = $("#xpsArriba").val();
        if  ($('#xpsCiudad').val() == 0){
            return false;
            }
		if((!(xpsAbajo) && !(xpsArriba)) || (xpsAbajo > 0 && xpsArriba > 0))
		{
	        $.ajax({
	            type: "post",
	            url: "../modules/TransporteCiudades/envioExpress.php",
	            data: {
	                "express": 'updateCiudad',
	                "ciudad": $("#xpsCiudad").val(),
	                "xpsabajo": xpsAbajo,
	                "xpsarriba": xpsArriba
	            },
	            success: function(response){
			...
			alert("Ambos valores deben estar vacíos o tener un costo mayor que cero para tomar los cambios");
		}
    }
...
{* <form action="{$current}&token={$token}&configure=TransporteCiudades" id="target"  method="post"> *}
    <fieldset><legend>{$transciuLogo}</legend>
        ...
        <div class="opciones" style="display:none"> <input type="radio" name="opc_ini" value="cargatransciudad"> Cargar precios ciudad - transportador </div>
        <div class="opciones" style="display:none"> <input type="radio" name="opc_ini" value="addtranscp"> Adicionar ciudades / colonias no registradas </div>
        <div class="opciones" style="display:none"> <input type="radio" name="opc_ini" value="actuesp"> Ajax ciudades </div>
        <div class="opciones"> <input type="radio" name="opc_ini" value="mediosp_ciudades"> Medios de pago por ciudades </div>
        <div class="opciones"> <input type="radio" name="opc_ini" value="envio_express"> Env&iacute;o Express</div>
        <div id="xps_state"style="display:none;float: left; width: 100%; margin: 20px;"> <input type="checkbox" id="express" value="1" onchange="expressEnabled();" {if $expressEnabled} checked{/if}> Habilitar env&iacute;o express
        <div id="xps_options" style="display:none; float: left; width: 100%; margin: 20px;">
        	<p>Departamento: &nbsp;
	        	<select id="xpsEstado" onchange="expressEstado();" {if !($expressEnabled)} disabled{/if}>
	            {foreach key=key item=item from=$EstadoDepto}
	                    <option value="{$key}"> {$item} </option> 
	            {/foreach}
	        	</select>
        	</p>
        	<p>Ciudad: &nbsp;
	        	<select id="xpsCiudad" onchange="expressCiudad();" disabled>
	        		<option value="" selected>-- Seleccione --</option>
	        	</select>
        	</p>
        	<p>Valor del env&iacute;o express para compras inferiores a ${$Umbral}:
        		<input type="number" id="xpsAbajo" onkeypress="return justNumbers(event);" disabled>
        	</p>
        	<p>Valor del env&iacute;o express para compras superiores a ${$Umbral}:
        		<input type="number" id="xpsArriba" onkeypress="return justNumbers(event);" disabled>
        	</p>
        	<a id="saveXps" alt="Guardar" onclick="guardarXps();" style="opacity: 0.4;">
        		<img src="themes/default/img/process-icon-save-and-stay.png" height="16px"/> Guardar
        	</a>
	        </div>
        </div>
            ...
            <form id="formcarga" action="../modules/TransporteCiudades/upload.php" method="post" enctype="multipart/form-data">
                <fieldset style ="width:80%"> 
                    ...
                    <p>Recuerde que el archivo CSV debe tener los siguientes campos  
                     ...
                     <br /><a href="../modules/TransporteCiudades/listado_ciudades_colonias.php"> ** Descarga AQUI el listado de ciudades y colonias ** </a><br />
                     <br />
