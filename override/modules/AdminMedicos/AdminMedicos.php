...
class AdminMedicos extends Module {
    ...
    public function getContent() {
        if ($this->json_empleado == '') {
            $this->json_empleado = ' "Empleado": {"id_employee" : "' . $this->context->employee->id . '", "nombres" : "' . $this->context->employee->firstname . '", "apellidos" : "' . $this->context->employee->lastname . '", "fecha" : "' . date('Y-m-d H:i:s') . '" }';
        }
        ...
        if (Tools::getValue('opc_ini')) {
            ...
            switch ($primeraopc) {
                ...
                case 'consulvisit':
                    if (Tools::getValue('visitadoroption') != '') {
                        $this->excelVisitadoresMedicos(Tools::getValue('visitadoroption'));
                    } else {
                        $this->output .= $this->adminDisplayWarning($this->l('No se enviaron todos los datos necesarios, Seleccione el visitador a consultar.' . $this->errores));
                        return $this->output . $this->displayForm();
                    }
                break;
            }
    ...
    public function excelVisitadoresMedicos($id_visitador){
        if ($id_visitador != ""){
            $sql = "SELECT med.id_medico, med.cedula, med.nombre, esp.nombre especialidad, CONCAT(emp.firstname,' ',emp.lastname) empleado
                    FROM "._DB_PREFIX_."medico med
                    INNER JOIN "._DB_PREFIX_."medic_especialidad medesp
                    ON med.id_medico = medesp.id_medico
                    INNER JOIN "._DB_PREFIX_."especialidad_medica esp
                    ON medesp.id_especialidad = esp.id_especialidad
                    INNER JOIN "._DB_PREFIX_."employee emp
                    ON med.id_visitador = emp.id_employee
                    WHERE emp.id_employee = ".$id_visitador."
                    ORDER BY med.nombre";
            require_once('reporteMedicos.php');
            $report = new reporteMedicos();
            if ($results = Db::getInstance()->ExecuteS($sql)) {
                $report -> download($results);
            } else {
                $sql2 ="SELECT  CONCAT(firstname,' ',lastname) empleado
                        FROM "._DB_PREFIX_."employee 
                        WHERE id_employee = ".$id_visitador."";
                $results2 = Db::getInstance()->ExecuteS($sql2);
                $report -> download($results2);
            }
        }
    }
    ...
    public function listaVisitadoresMedicos() {
        ...
        $visitadoresmedicos_lista[''] = ' -- Seleccione -- ';

        if ($results = Db::getInstance()->ExecuteS($sql)) {
            foreach ($results as $valores) {
                $visitadoresmedicos_lista[$valores['id_employee']] = $valores['firstname'] . " " . $valores['lastname'];
            }
        }

        $this->context->smarty->assign(array('visitadoresmedicos' => $visitadoresmedicos_lista));
    }
    ...
    public function cargaMedicoPorIdCupon($id_cupon) {
        ...
        //echo "<br>".
        $sql = 'SELECT m.*, GROUP_CONCAT( me.id_especialidad ) especialidad FROM '._DB_PREFIX_.'medico m
			LEFT JOIN '._DB_PREFIX_.'medico_rule mr ON ( mr.id_medico = m.id_medico )
			LEFT JOIN '._DB_PREFIX_.'medic_especialidad me ON ( me.id_medico = m.id_medico )
        ...
        if ($results = Db::getInstance()->ExecuteS($sql)) {
            ...
            $this->context->smarty->assign('id_visitador', $valores['id_visitador']);
            $this->context->smarty->assign('tipo_cuenta', $valores['tipo_cuenta']);
            $id_medico = $valores['id_medico'];
        }
        
        $sqlLogVisitador = "SELECT Date_format(log.fecha,'%d-%m-%Y') AS fecha, CONCAT(emp.firstname,' ',emp.lastname) AS name_visitador
                            FROM "._DB_PREFIX_."medico_visitador_log AS log
                            LEFT JOIN "._DB_PREFIX_."employee AS emp
                            ON log.id_visitador = emp.id_employee
                            WHERE log.id_medico = ".$id_medico."
                            ORDER BY log.fecha DESC";
        if ($results = Db::getInstance()->ExecuteS($sqlLogVisitador)) {
            $penvis = $results[0]['name_visitador'];
            $antpenvis = $results[1]['name_visitador'];
            if ($results[0]['name_visitador'] == "") {
                $penvis = "Sin visitador";
            }
            if ($results[1]['name_visitador'] == "") {
                $antpenvis = "Sin visitador";
            }
            $this->context->smarty->assign('ultimaFecha', $results[0]['fecha']);
            $this->context->smarty->assign('penVisitador', $penvis);
            $this->context->smarty->assign('antpenVisitador', $antpenvis);
        } else {
            $this->context->smarty->assign('ultimaFecha', "--");
            $this->context->smarty->assign('penVisitador', "--");
            $this->context->smarty->assign('antpenVisitador', "--");
        }
    }
...
//echo $sql;
    ...
    public function registernewmed() {
        ...
        $campos_a_guardar = array("id_medico", "nombre", "domicilio", "telefono", "localidad", "region", "contrato", "activo", "banco", "num_cuenta",
            "beneficio", "cedula", "email", "valor_comision", "porc_rango_menor", "porc_rango_mayor", "observacion", "beneficiario", "id_visitador", "tipo_cuenta");
                ...
                if ($results_ins_rule_lang = Db::getInstance()->Execute($ins_rule_lang)) {
                    ...
                    if ($result_ins_espe_med = Db::getInstance()->ExecuteS($ins_espe_med)) {
                        ...
                        if ($results_ins_regla_medico = Db::getInstance()->Execute($ins_regla_medico)) {
                            ...
                            //INSERTAR LOG DE COMISIONES ENTREGADAS A VISITADORES
                            $this->registerLogMedVis($this->bd_fielsd_save['id_medico'], $this->bd_fielsd_save['id_visitador'], "I");
        ...
        if ($result_old_datos = Db::getInstance()->ExecuteS($query_old_datos)) {
            ...
            foreach ($campos_a_modificar as $key => $value) {
                if ($value != "id_medico") {
                    ...
                    if ($value == 'id_visitador' && $this->permisos_basicos != 3) {

                        $query_datos_up .= " " . $value . " = '" . strtoupper($this->bd_fielsd_save[$value]) . "',";                        
                        $visitador = $this->bd_fielsd_save[$value];

                    } elseif ($value == 'id_visitador' && $this->permisos_basicos == 3 ) {

                    } elseif ($this->bd_fielsd_save[$value] == '') {
            ...
            if ($result_datos_up = Db::getInstance()->ExecuteS($query_datos_up)) {
                ...
                if ($result_del_especialidad = Db::getInstance()->ExecuteS($query_del_especialidad)) {
                    ...
                    if ($result_ins_espe_med = Db::getInstance()->ExecuteS($ins_espe_med)) {
                        ...
                        //INSERTAR LOG DE COMISIONES ENTREGADAS A VISITADORES
                        if ($result_old_datos[0]['id_visitador'] != $visitador && $this->permisos_basicos != 3){
                            $this->registerLogMedVis($this->bd_fielsd_save['id_medico'], $result_old_datos[0]['id_visitador'], "U");
                        }
    ...
    //LOG DE COMISIONES ENTREGADAS A VISITADORES
    public function registerLogMedVis($id_medico, $id_visitador, $action) {
        if ($id_visitador == "") {
            $id_visitador = "NULL";
        }
        $ins_log = "INSERT INTO " . _DB_PREFIX_ . "medico_visitador_log (id_medico,id_visitador,action,fecha,id_employee) VALUES (" . $id_medico . "," . $id_visitador . ",'" . $action . "','" . date("Y-m-d H:i:s") . "'," . $this->context->employee->id . ")";
        Db::getInstance()->Execute($ins_log);
    }
...
// Carga listado de los bancos
    public function listaBancos() {
        $sql = 'SELECT name_bank FROM ps_bank ORDER BY name_bank';
        $banco_lista[''] = ' - Seleccione Entidad - ';
        $i = 0;
        if ($results = Db::getInstance()->ExecuteS($sql)) {
            foreach ($results as $valores) {
                $banco_lista[$valores['name_bank']] = $valores['name_bank'];
            }
        }
        return $banco_lista;
    }
...
// Muestra el formulario de creación de cupones médicos
    public function displayCreaCupon() {
        $str_nombancos = $this->listaBancos();
        $this->context->smarty->assign('listbancos', $str_nombancos);
        $this->listaEmpleadosVisitadores();
        $this->context->smarty->assign('si_no', array('' => ' -- Seleccione -- ', 'si' => 'si', 'no' => 'no'));
        $this->context->smarty->assign('si_no_cede', array('' => ' -- Seleccione -- ', 'si' => 'si', 'no' => 'no', 'cedido' => 'cedido'));
        $this->context->smarty->assign('corriente_ahorros', array('' => ' -- Seleccione -- ', 'CORRIENTE' => 'Corriente', 'AHORROS' => 'Ahorros'));
