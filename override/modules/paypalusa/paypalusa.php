...
class PayPalUSA extends PaymentModule
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l((Validate::isLoadedObject($this->_shop_country) && $this->_shop_country->iso_code == 'MX') ? 'PayPal' : 'PayPal');
	...
	{  
		...
		if (Validate::isLoadedObject($this->_shop_country) && !in_array($this->_shop_country->iso_code, array('US', 'MX', 'CA','CO')))
		{
	...
	public function hookBackOfficeHeader()
	{
		...
		if (!isset($_GET['vieworder']) || !isset($_GET['id_order']))
			return;
                ...
                if(Utilities ::available_property('refund_amount',$this->context->employee->id,'configure')){
                    $this->context->smarty->assign('paypal_usa_refund_avilable', TRUE);
                }else{
                  $this->context->smarty->assign('paypal_usa_refund_avilable', FALSE);    
                }
	...
	private function _processRefund($id_transaction, $amount, $original_transaction)
	{
		...
		if ($original_transaction['source'] == 'express' || ($original_transaction['source'] == 'standard' && Configuration::get('PAYPAL_USA_ACCOUNT') && Configuration::get('PAYPAL_USA_ACCOUNT') && Configuration::get('PAYPAL_USA_ACCOUNT')) && Utilities ::available_property('refund_amount',$this->context->employee->id,'configure'))
		{
			...
			if (Tools::strtoupper($result['ACK']) == 'SUCCESS' || Tools::strtoupper($result['ACK']) == 'SUCCESSWITHWARNING')
			{
				...
				$this->context->smarty->assign('paypal_usa_refund', 1);
                                ...
                                $order = new Order((int) (int)Tools::getValue('id_order'));
                                $order->setCurrentState((int) Configuration::get('PS_OS_CANCELED'),(int) $this->context->employee->id);
