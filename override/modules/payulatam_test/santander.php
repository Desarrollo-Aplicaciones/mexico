...
class Santander extends PayUControllerWS{    
...
if (isset($_POST['pagar']))
  {
    ...
    $fechaSANTANDER=date ( 'Y-m-d' , $nuevafecha ).'T'.date ( 'h:i:s' , $nuevafecha );          
    $keysPayu= $conf->keys();

// Script Json payuLatam (SANTANDER)   
...
if($response['code'] === 'ERROR')
{
 ...
 $conf->failed_transaction(0.0, $customer->id, $data, $response, 'SANTANDER', $response['code'], $this->context->cart->id,$id_address);
 $messsage='<h1>¡Error!, Transaccion Rechazada</h1><br>'.$response['error'];    
    ...
    elseif ($response['code'] === 'SUCCESS' && $response['transactionResponse']['state'] === 'PENDING' && $response['transactionResponse']['responseMessage'] != 'ERROR_CONVERTING_TRANSACTION_AMOUNTS') {
            ...
            $extra_vars =  array('method'=>'SANTANDER',
                                 'cod_pago'=>$response['transactionResponse']['extraParameters']['REFERENCE'],
                                 'fechaex'=> date('Y-m-d',$nuevafecha),
                                 'bar_code'=>$response['transactionResponse']['extraParameters']['BAR_CODE']);

            $this->createPendingOrder($extra_vars, 'SANTANDER', 'El sistema esta en espera de la confirmación de la pasarela de pago.');
            ...
            $conf->pago_payu($order['id_order'], $customer->id, $data, $response, 'SANTANDER', $extras, $id_cart,$id_address);
     ...
     $conf->failed_transaction(0.0, $customer->id, $data, $response, 'SANTANDER', $response['transactionResponse']['state'], $id_cart,$id_address);
