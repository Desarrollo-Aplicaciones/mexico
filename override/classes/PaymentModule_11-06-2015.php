...
class PaymentModule extends PaymentModuleCore {
    ...
    public function validateOrder($id_cart, $id_order_state, $amount_paid, $payment_method = 'Unknown', $message = null, $extra_vars = array(), $currency_special = null, $dont_touch_amount = false, $secure_key = false, Shop $shop = null) {
        ...
        $this->logtxt('1) id_cart: '.$id_cart.',  id_order_state: '.$id_order_state.", payment_method: ".$payment_method);
...
//<td colspan="4" style="padding:0.6em 0.4em;text-align:right">'.Tools::displayError('Voucher name:').' '.$cart_rule['obj']->name.'</td>
                    ...
                    foreach ($this->context->cart->getProducts() as $product)
                        ...
                        if (Configuration::get('PS_STOCK_MANAGEMENT') && $order_detail->getStockState()) 
                        {
                            ...
                            $this->logtxt('3)  order->id: '.$order->id.',  history->id_order_state: '.$history->id_order_state.',  order->current_state: '.$order->current_state.", payment_method: ".$payment_method);
                    ...
                    $new_history->changeIdOrderState((int) $id_order_state, $order, true);
                     $this->logtxt('4)  order->id: '.$order->id.',  history->id_order_state: '.$new_history->id_order_state.',  order->current_state: '.$order->current_state.", payment_method: ".$payment_method);
            ...
            $this->logtxt('5)  order->id: '.$order->id.',  order->current_state: '.$order->current_state.", payment_method: ".$payment_method);
            return true;
        } else {
