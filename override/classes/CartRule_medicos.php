...
public function getAssociatedRestrictions($type, $active_only, $i18n)
    {
        ...
        {
            if ($type == 'cart_rule') { 
                //..echo "<br>over 4 si es carrito";
                $array = $this->getCartRuleCombinations();
            } else {
                //..echo "<br>over 5 si no es carrito";
     ...
     * @return array
     */
    public static function getCartsRuleByNameDoc($name, $id_lang, $visitador)
    {
        ...
        $query = "SELECT cr.id_cart_rule, cr.id_customer, cr.date_from, cr.date_to, cr.description, cr.quantity, cr.quantity_per_user, cr.priority, 
            cr.partial_use, IF(CONCAT('[',GROUP_CONCAT(esm.nombre SEPARATOR \", \"),']') IS NULL, '[ - ]', CONCAT('[',GROUP_CONCAT(esm.nombre SEPARATOR \", \"),']') ) AS code, 
            cr.minimum_amount, cr.minimum_amount_tax, 
