...
//                fclose($fp) ;
    ...
    public function validateOrder($id_cart, $id_order_state, $amount_paid, $payment_method = 'Unknown', $message = null, $extra_vars = array(), $currency_special = null, $dont_touch_amount = false, $secure_key = false, Shop $shop = null, $private_message = null) {
        ...
        $this->context->cart = new Cart($id_cart, null, $this->context->cart->removeRulesGroup);
...
die();*/
                    ...
                    $opera = 0;
				if(Context::getContext()->cookie->check_xps && (!isset(Context::getContext()->cookie->entrega_nocturna) || Context::getContext()->cookie->entrega_nocturna === 'disabled'))
					{
						Db::getInstance()->insert('orders_transporte', array(
						'id_order' => $order->id,
						'extras'      => 'express',
						));
                                                unset(Context::getContext()->cookie->entrega_nocturna);
                                                unset(Context::getContext()->cookie->check_xps);
					}
                                if(isset(Context::getContext()->cookie->entrega_nocturna) && Context::getContext()->cookie->entrega_nocturna === 'enabled' && !Context::getContext()->cookie->check_xps)
					{
						Db::getInstance()->insert('orders_transporte', array(
						'id_order' => $order->id,
						'extras'      => 'entrega_nocturna',
						));
                                                unset(Context::getContext()->cookie->entrega_nocturna);
                                                unset(Context::getContext()->cookie->check_xps);
                                        }       
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (!$order_creation_failed && isset($order->id)) {
                        ...
                        if (!$customization_quantity || (int) $product['cart_quantity'] > $customization_quantity)
                            $products_list .=
                                    '<tr style="background-color: ' . ($key % 2 ? '#DDE2E6' : '#EBECEE') . ';">
                                                ...
                                                <td style="font-size: 10px; padding: 0.6em 0.4em;width: 30%;"><strong>' . $product['name'] . (isset($product['attributes']) ? ' - ' . $product['attributes'] : '') . '</strong></td>
                    ...
                    foreach ($cart_rules as $cart_rule) {
                        ...
                        // SE INHABILITA PARA PODER ALMACENAR EL CUPON DE DESCUENTO CON VALORES EN 0
...
//<td colspan="4" style="padding:0.6em 0.4em;text-align:right">'.Tools::displayError('Voucher name:').' '.$cart_rule['obj']->name.'</td>
                ...
                $metodo_de_pago = '
                                ...
                                if(isset($extra_vars['convenio'])){
                                $metodo_de_pago.='<td align="left">'.$extra_vars['convenio']['label'].' <strong>' .$extra_vars['convenio']['value'] . '</strong></td>
                                </tr>';
                                }                                                                
                $metodo_de_pago.='<tr>
