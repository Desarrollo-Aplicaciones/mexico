...
class Cart extends CartCore {
	...
	{
                            $total_products_recalculado = NULL;
		...
		**************/
		...
		if ( isset($cartRules[0]) && !empty($cartRules[0]) && $cartRules[0] != "" ) {
			...
			if ( $free_shipping == 1 ) {
				...
				$total_shipping = 0;
				$total_shipping_tax_exc = 0;
				$cart_rules[0]['value_real'] = 0;
				$cart_rules[0]['value_tax_exc'] = 0;
			}
		}
		...
		///*** INICIO CALCULO COSTO TOTAL, CON CUPON DE DESCUENTO PORCENTAJE ***///
		$cartRules = $this->getCartRules();
        if(isset($cartRules[0]['reduction_percent']) && !empty($cartRules[0]['reduction_percent'])){
		...
		///*** INICIO CALCULO COSTO TOTAL, CON CUPON DE DESCUENTO MONETARIO ***///
		if ( isset($cartRules[0]['reduction_amount']) && !empty($cartRules[0]['reduction_amount']) ) {
		...
		if ($descuentoMoney != "" && $descuentoMoney != 0) {
			$descuentoMonetario = $this->RecalcularCuponMonetario();

			$base_total_tax_inc = $descuentoMonetario['totales']['total_orden'];
			$total_tax = $descuentoMonetario['totales']['total_iva_pesos_grupal'];
			$cart_rules[0]['value_real'] = $descuentoMonetario['totales']['descuento_aplicado'];
			$total_discounts_tax_exc = $descuentoMonetario['totales']['descuento_aplicado'];
			$base_total_tax_exc = $descuentoMonetario['totales']['total_orden'] - $descuentoMonetario['totales']['total_iva_pesos_grupal'];
...
$total_iva=0;
 ...
 //exit('<br> <br> <pre>'.print_r($this->getCartRules(),true));
