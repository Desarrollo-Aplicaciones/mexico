...
class OrderDetail extends OrderDetailCore {
	...
	protected function create(Order $order, Cart $cart, $product, $id_order_state, $id_order_invoice, $use_taxes = true, $id_warehouse = 0)
	{
		...
		$descuento_porcentaje = $cartRules[0]['reduction_percent'];
		$descuento_monetario = $cartRules[0]['reduction_amount'];

		if ($descuento_monetario != "" && $descuento_monetario != 0){
			$this->reduction_amount = $descuento_monetario;
		}
		...
		if ($descuento_porcentaje != "" && $descuento_porcentaje != 0){
			...
			$descuento_sin_iva = ($precio * $descuento_porcentaje) / 100;
			$this->iva_producto_recalculado = ( $precio - (( $precio * $descuento_porcentaje) / 100 )) * ($iva / 100);
			...
			*/
			...
			$this->reduction_percent = $descuento_porcentaje;
