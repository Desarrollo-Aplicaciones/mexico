...
class Address extends AddressCore
{
    ...
    /**
     * [$colonia_name Nombre de la colonia]
     * @var string
     */
    public $colonia_name = '';
    ...
    public static $definition = array(
                                      ...
                                      'fields' => array(
                                                        ...
                                                        'alias' =>              array('type' => self::TYPE_STRING, 'validate' => 'isGenericName', 'required' => true, 'size' => 128),
                                                        ...
                                                        'is_rfc' =>             array('type' => self::TYPE_BOOL),
                                                        ),
);
...
public  function __construct($id_address = null, $id_lang = null)
{
    ...
    if($id_address) {
        ...
        if(isset($result) && !empty($result) && count($result)>0 ) {
            ...
            if (isset(Context::getContext()->employee->id)) {
                $this->city_id = $result[0]['city_name'];
            }
	...
	public function add($autodate = true, $null_values = false)
	{
       ...
       if ($this->id_colonia 
          ...
          $val_cityid = $this->getIdCityFromIdColony($this->id_colonia);
       }
   ...
   return true;
    ...
    public function getIdCityFromIdColony($idPostCode = 0)
    {
        $sql = "SELECT id_ciudad FROM "._DB_PREFIX_."cod_postal WHERE id_codigo_postal = " . (int)$idPostCode;
...
public static function get_list_address_app($id_customer) {

    $val_express = Configuration::get('EXPRESS') ? Configuration::get('EXPRESS') : 0;

    if ($id_customer !== '') {

        $query = "SELECT address.id_address, address.alias, address.postcode, address.address1,address.address2, cities.city_name, codp.nombre as colonia, 
        state.`name` AS state_name, country.`name` AS country_name,
        cac.precio_kilo, car.id_carrier, 
        SUBSTRING(REPLACE( crp.delimiter2,'.',','),1,
                  LENGTH(REPLACE( crp.delimiter2,'.',',')) -7) AS delimiter2,
...
LEFT JOIN "._DB_PREFIX_."cod_postal codp ON(codp.id_codigo_postal = address.id_colonia)
INNER JOIN "._DB_PREFIX_."range_price crp ON (crp.id_carrier = car.id_carrier)
WHERE customer.id_customer = ".(int) $id_customer;
$arr_dirs_user = array();
