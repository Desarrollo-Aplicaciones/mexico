...
class DiscountController extends DiscountControllerCore
{
    ...
    public function initContent()
    {
        if (Module::isEnabled('quantitydiscountpro')) {
            ...
            $quantityDiscountRules = $quantityDiscount->getQuantityDiscountRulesAtCart($this->context->cart->id);

            $cart_rules = CartRule::getCustomerCartRules($this->context->language->id, $this->context->customer->id, true, false);

            if (is_array($quantityDiscountRules) && array_filter($quantityDiscountRules)) {
                foreach ($cart_rules as $key => &$discount) {
                    if (in_array($discount['id_cart_rule'], array_column($quantityDiscountRules, 'id_cart_rule'))) {
