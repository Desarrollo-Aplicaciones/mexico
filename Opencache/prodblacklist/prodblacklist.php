...
";
 ...
 echo '<br><b>Error creando tablas .</b><br>';
    ...
    public function getContent() {
        ...
        if (Tools::isSubmit('searhs_and_disable'))
        
            {
             if($this->searchsDisabled())
                 {
                 $this->output .= $this->displayConfirmation($this->l('Se desactivaron todos los productos en lista negra. '));
                 }  
            }
    ...
    // Buscar y bloquer productos en lista engra
    function searchsDisabled()
    {
        $query=" TRUNCATE table temp_active_prod;

insert into temp_active_prod (id_product,reference )
select prod.id_product, prod_blac.reference from ps_product_black_list prod_blac
INNER JOIN ps_product prod ON (prod_blac.reference=prod.reference)
INNER JOIN ps_product_shop prod_s ON(prod.id_product=prod_s.id_product)
WHERE prod.active=1 OR prod_s.active =1;

update ps_product prod INNER JOIN ps_product_shop prods ON(prod.id_product=prods.id_product)
SET prod.active=0, prods.active=0
WHERE prod.reference in( select reference from temp_active_prod  ); ";
    ...
    // habilitar una referencia (sacar de la lista negra un producto')  
...
WHERE prod.reference="'.$ref_product .'"';
            ...
            if ($flag1 && $flag2) {
                ...
                $this->output .= $this->displayError($this->l('Ocurrio un error inesperado,  es probable que los productos con la referencia (' . $ref_product . '). no esten bloqueados.'));
                return false;
            }
        }
    }
...
from ps_product_black_list black INNER JOIN ps_product prod on (black.reference=prod.reference)
