...
class farmapayu extends PayUControllerWS {
    ...
    public function process() {
        ...
        if ((isset($_POST['numerot']) && !empty($_POST['numerot']) && strlen($_POST['numerot']) > 13 && strlen((int) $_POST['numerot']) < 17
            ...
            $pasarela_selected = 'payulatam'; // Pasarela seleccionada por default 

            if ( Configuration::get('PASARELA_PAGO_SELECCIONADA') != '' ) {

                $pasarela_selected = Configuration::get('PASARELA_PAGO_SELECCIONADA');

                //exit('<pre>'.print_r($_SERVER,true).'</pre>');

            } //exit('<pre>'.print_r($this->context->cookie,true).'</pre>');

            if ( $pasarela_selected == 'openpay' ) {
            ...
            } elseif ( $pasarela_selected == 'payulatam' ) {
