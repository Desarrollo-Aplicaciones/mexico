...
class PayUController extends FrontController
{
	...
	public function initParams()
	{
        ...
        $cartRules = self::$cart->getCartRules();
        $free_shipping = $cartRules[0]['free_shipping'];
        $descuento = $cartRules[0]['reduction_percent'];

        //tomar costo de envio
        $price_shipping = self::$cart->getTotalShippingCost();
        $validateaddress = self::$cart->validationaddressfarmalisto();
        if ( $validateaddress || $free_shipping == 1 ) {
            $price_shipping = 0;
        }
        ...
        if ($descuento != "" && $descuento != 0){
           $totalorderdescuento = self::$cart->recalculartotalconcupon($descuento);
           $totalorderdescuento['total'] = $totalorderdescuento['total'] + $price_shipping;
           $params[4]['amount'] = Tools::ps_round((float)$totalorderdescuento['total'], 2);
        }
        ...
        if ( $descuentoMoney != "" && $descuentoMoney != 0 ) {
            $descuentoMonetario = self::$cart->RecalcularCuponMonetario();
            
            $params[4]['amount'] = Tools::ps_round((float)$descuentoMonetario['totales']['total_orden'], 2);
        }
