...
&& in_array($extension, $allowedExts))
  ...
  else
    {
     ...
     if (is_array($names) && $names[0] != '' && $names[0] != false && $names[2] !=false ){
     	...
     	if ($retorno_cargue == true) {
	     	...
	     	&& $guardar_archivo->validarIcrCargadoVsSupplyOrderIcrCantidades() && $guardar_archivo->validarEstadoRegistrosCargadosEntrada() && $guardar_archivo->OrdenesProductosEntrada()  
 ...
 return $output.$this->displayForm();
	...
	<p>Con este modulo usted podrá actualizar las ordenes de entrada, recuerde que el archivo CSV debe tener los siguientes campos  <a download href="../modules/loadicrentrada/formato.csv">(ORDEN_SUMINISTRO, PROVEEDOR, EAN, DESCRIPCION, ICR, FECHA, CANTIDAD, PRECIO_COMPRA, IVA, LOTE, FECHA_VENCIMIENTO)</a>, estos <b>deben estar</b> en la cabecera del archivo.</p>
