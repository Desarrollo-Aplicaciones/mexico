...
class csstaticblocks extends Module
{
	...
	private $_html;
	private $myHook = array('displayTop','displayLeftColumn','displayRightColumn','displayFooter','displayHome','homebottom','footertop','footerbottom', 'FAQ');
	...
	public function hookFAQ()
	{
		...
		{
			$smarty_cache_id = $this->getCacheId('csstaticblocks_FAQ');
		}
		if (!$this->isCached('csstaticblocks_FAQ.tpl', $smarty_cache_id))
		{
			$block_list = $this->getBlockInHook('FAQ');
			$smarty->assign(array(
				'block_list' => $block_list
			));
		}
		if (version_compare(_PS_VERSION_,'1.5.4','<'))
			Tools::restoreCacheSettings();
		return $this->display(__FILE__, 'csstaticblocks_FAQ.tpl', $smarty_cache_id);
	}
