...
class PayUControllerWS extends FrontController {
    ...
    public function initParams() {
        ...
        $params = array(
            ...
            array('amount' => $this->context->cart->getOrderTotals()['total_orden'], 'name' => 'amount'),
    ...
    public function createPendingOrder($extra_vars = array(), $metodo_de_pago, $mensaje) {
        try {
                $date = date("Y-m-d H:i:s");
                    $sql="INSERT INTO "._DB_PREFIX_."sonda_payu (id_cart,date_add,`interval`,last_update)
                    VALUES(".(int)$this->context->cart->id.",'".$date."',";
                        if($metodo_de_pago === 'Tarjeta_credito'){
                             $sql.=11;         
                        }else{
                            $sql.=61; 
                        }
                    $sql.=", '".$date."');"; 
                    //exit('<pre>'.print_r($sql,true));   
                    if(!Db::getInstance()->Execute($sql))
                        Logger::AddLog('Error al guardar sonda_payu id_cart: '.$this->context->cart->id, 2, null, null, null, true);
       

            $payu = new PayULatam();
            $payu->validateOrder((int) self::$cart->id, (int) Configuration::get('PAYU_WAITING_PAYMENT'), (float) self::$cart->getOrderTotal(), $metodo_de_pago, $mensaje, $extra_vars, NULL, false, self::$cart->secure_key);
        } catch (Exception $e) {
