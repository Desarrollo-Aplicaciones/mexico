...
class Ixe extends PayUControllerWS{    
...
if (isset($_POST['pagar']))
  {
    ...
    $fechaIXE=date ( 'Y-m-d' , $nuevafecha ).'T'.date ( 'h:i:s' , $nuevafecha );          
    $keysPayu= $conf->keys();

// Script Json payuLatam (IXE)   
...
if($response['code'] === 'ERROR')
{
 ...
 $conf->failed_transaction(0.0, $customer->id, $data, $response, 'IXE', 'code=ERROR', $this->context->cart->id,$id_address);
 $messsage='<h1>¡Error!, Transaccion Rechazada</h1><br>'.$response['error'];    
            ...
            elseif ($response['code'] === 'SUCCESS' && $response['transactionResponse']['state'] === 'PENDING' && $response['transactionResponse']['responseMessage'] != 'ERROR_CONVERTING_TRANSACTION_AMOUNTS') {
                    ...
                    $this->createPendingOrder($extra_vars, 'IXE', 'El sistema esta en espera de la confirmación de la pasarela de pago.');
                    ...
                    $conf->pago_payu($order['id_order'], $customer->id, $data, $response, 'IXE', $extras, $id_cart,$id_address);
     ...
     $conf->failed_transaction(0.0, $customer->id, $data, $response, 'IXE', $response['transactionResponse']['state'], $id_cart,$id_address);
