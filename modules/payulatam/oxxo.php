<?php
        ...
        require_once(realpath(_PS_MODULE_DIR_.'payulatam/payulatam.php'));
        require_once(realpath(_PS_MODULE_DIR_.'payulatam/config.php'));
        require_once(realpath(_PS_MODULE_DIR_.'payulatam/paymentws.php'));
            ...
            public function process()
            {
                    ...
                    if (isset($_POST['pagar'])) {
                        ...
                        if($response['code'] === 'ERROR') {
                                $conf->error_payu($id_order, $customer->id, $data, $response, 'OXXO', $response['transactionResponse']['state'], $this->context->cart->id, $id_address); 
                        ...
                        elseif ($response['code'] === 'SUCCESS' && $response['transactionResponse']['state'] === 'PENDING' && $response['transactionResponse']['responseMessage'] != 'ERROR_CONVERTING_TRANSACTION_AMOUNTS') {
                                ...
                                $this->createPendingOrder($extra_vars, 'OXXO', 'El sistema esta en espera de la confirmación de la pasarela de pago.', 'PAYU_WAITING_PAYMENT');
