...
$output .= $this->displayConfirmation("<br>La cache de PHP (OPcache)se eliminara a mas tardar en un minuto.<br>");
	...
	{
		$this->context->controller->addCSS(($this->_path).'limpiarcache.css', 'all');
	}

	public function hookDisplayHome($params)
	{
		if (!$this->isCached('limpiarcache.tpl', $this->getCacheId('limpiarcache')))
		{
			$category = new Category(Context::getContext()->shop->getCategory(), (int)Context::getContext()->language->id);
			$nb = (int)Configuration::get('HOME_FEATURED_NBR');

		}
		return $this->display(__FILE__, 'limpiarcache.tpl', $this->getCacheId('limpiarcache'));
	}
