...
class CashondeliveryValidationModuleFrontController extends ModuleFrontController
{
	...
	{ 
		if (empty($this->context->cart->id) || $this->context->cart->id_customer == 0 || $this->context->cart->id_address_delivery == 0 || $this->context->cart->id_address_invoice == 0 || !$this->module->active)
		...
		if (Tools::getValue('confirm'))
		{
			...
			$total = $this->context->cart->getOrderTotal(false, Cart::BOTH);
			$cod_pagar = Tools::getValue('cod_pagar');
			$this->module->validateOrder((int)$this->context->cart->id, Configuration::get('PS_OS_PREPARATION'),$total, !empty($cod_pagar) ? $cod_pagar : $this->module->displayName, null, array(), null, false, $customer->secure_key);
