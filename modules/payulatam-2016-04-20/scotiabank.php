...
';
            ...
            if ($response['code'] === 'ERROR') {
                ...
                $conf->error_payu($id_order, $customer->id, $data, $response, 'SCOTIABANK', $response['transactionResponse']['state'], $this->context->cart->id, $id_address); 
            ...
            elseif ($response['code'] === 'SUCCESS' && $response['transactionResponse']['state'] === 'PENDING' && $response['transactionResponse']['responseMessage'] != 'ERROR_CONVERTING_TRANSACTION_AMOUNTS') {
                ...
                $convenio = array('label' => 'Servicio No.:','value'=>'3198' );
                ...
                $this->createPendingOrder($extra_vars, 'SCOTIABANK', 'El sistema esta en espera de la confirmación de la pasarela de pago.', 'PAYU_WAITING_PAYMENT');
                ...
                $conf->pago_payu($order['id_order'], $customer->id, $data, $response, 'SCOTIABANK', $extras .';'.json_encode($convenio), $id_cart, $id_address);
