...
defaults('USE_AUTHENTICATION',1);			// Use (internal) authentication - best choice if 
											...
											//  this work!
defaults('ADMIN_USERNAME','juan'); 			// Admin Username
defaults('ADMIN_PASSWORD','topo');  	// Admin Password - CHANGE THIS TO ENABLE!!!
...
if (isset($MYREQUEST['IMG']))
{
	...
	function fill_arc($im, $centerX, $centerY, $diameter, $start, $end, $color1,$color2,$text='',$placeindex=0) {
		$r=$diameter/2;
		$w=deg2rad((360+$start+($end-$start)/2)60);
  ...
  echo
	...
	foreach($cache[$scope_list[$MYREQUEST['SCOPE']]] as $i => $entry) {
		switch($MYREQUEST['SORT1']) {
			case 'A': $k=sprintf('5d-',$entry['access_time']); 	break;
			case 'H': $k=sprintf('5d-',$entry['num_hits']); 		break;
			case 'Z': $k=sprintf('5d-',$entry['mem_size']); 		break;
			case 'M': $k=sprintf('5d-',$entry['mtime']);			break;
			case 'C': $k=sprintf('5d-',$entry['creation_time']);	break;
			case 'T': $k=sprintf('5d-',$entry['ttl']);			break;
			case 'D': $k=sprintf('5d-',$entry['deletion_time']);	break;
			case 'S': $k='';										break;
		}
		if (!$AUTHENTICATED) {
			// hide all path entries if not logged in
...
EOB;
	...
	foreach ($tmp as $k => $v) {
		switch($MYREQUEST['SORT1']) {
			case 'A': $kn=sprintf('5d-',$v['size'] / $v['ents']);break;
			case 'T': $kn=sprintf('5d-',$v['ents']);		break;
			case 'H': $kn=sprintf('5d-',$v['hits']);		break;
			case 'Z': $kn=sprintf('5d-',$v['size']);		break;
			case 'C': $kn=sprintf('5d-',$v['hits'] / $v['ents']);break;
			case 'S': $kn = $k;					break;
		}
